...
void test_pcep_tlv_create_path_setup_type_capability(void)
{
	...
	*pst3 = 3;
	...
	struct pcep_object_tlv_header *sub_tlv1 =
		(struct pcep_object_tlv_header *)pcep_tlv_create_sr_pce_capability(true, true, 0);
	struct pcep_object_tlv_header *sub_tlv2 =
		(struct pcep_object_tlv_header *)pcep_tlv_create_srv6_pce_capability(true, 3, 4, 5,
										     6);
	...
	*pst2 = SRV6_TE_PST;
	...
	CU_ASSERT_EQUAL(tlv->header.type,
			PCEP_OBJ_TLV_TYPE_PATH_SETUP_TYPE_CAPABILITY);
	CU_ASSERT_EQUAL(tlv->header.encoded_tlv_length, sizeof(uint32_t) * 3 + TLV_HEADER_LENGTH +
	...
	CU_ASSERT_EQUAL(uint16_ptr[0], htons(PCEP_OBJ_TLV_TYPE_SRV6_PCE_CAPABILITY));
