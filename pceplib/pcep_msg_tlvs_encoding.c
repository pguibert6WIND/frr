...
uint16_t (*const tlv_encoders[MAX_TLV_ENCODER_INDEX])(PCEP_TLV_ENCODERS_ARGS) = {
	...
	[PCEP_OBJ_TLV_TYPE_SRV6_PCE_CAPABILITY] = pcep_encode_tlv_srv6_pce_capability,
...
struct pcep_object_tlv_header *(*const tlv_decoders[MAX_TLV_ENCODER_INDEX])(
	...
	[PCEP_OBJ_TLV_TYPE_SRV6_PCE_CAPABILITY] = pcep_decode_tlv_srv6_pce_capability,
					     ...
					     uint8_t *tlv_body_buf)
{
	...
	tlv_body_buf[i] = srv6_pce_cap->flag_n ? TLV_SRV6_PCE_CAP_FLAG_N : 0x00;
	...
	if (srv6_pce_cap->msd_segs_left) {
		tlv_body_buf[++i] = SRV6_MSD_TYPE_SEGS_LEFT;
				    ...
				    const uint8_t *tlv_body_buf)
{
	...
	do {
		...
		else if (tlv_body_buf[i] == SRV6_MSD_TYPE_SEGS_LEFT)
...
struct pcep_object_tlv_header *
pcep_decode_tlv_path_setup_type(struct pcep_object_tlv_header *tlv_hdr, const uint8_t *tlv_body_buf)
{
	struct pcep_object_tlv_path_setup_type *tlv =
