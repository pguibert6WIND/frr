...
struct pcep_message *create_pcep_open(pcep_session *session)
{
	...
	if (session->pcc_config.support_sr_te_pst || session->pcc_config.support_srv6_te_pst)
	...
	if (session->pcc_config.support_srv6_te_pst) {
		...
		if (session->pcc_config.pcep_msg_versioning->draft_ietf_pce_segment_routing_07 ==
		    false) {
			flag_n = session->pcc_config.pcc_can_resolve_nai_to_ipv6_sid;
		}

		struct pcep_object_tlv_srv6_pce_capability *srv6_pce_cap_tlv =
			pcep_tlv_create_srv6_pce_capability(flag_n,
							    session->pcc_config
								    .max_sid_depth_srv6_end_d,
							    session->pcc_config
								    .max_sid_depth_srv6_end_pop,
							    session->pcc_config
								    .max_sid_depth_srv6_h_encaps,
							    session->pcc_config
								    .max_sid_depth_srv6_segs_left);
	...
	if (session->pcc_config.support_sr_te_pst || session->pcc_config.support_srv6_te_pst)
		dll_append(tlv_list,
			   pcep_tlv_create_path_setup_type_capability(pst_list, sub_tlv_list));
