...
void cli_show_router_isis(struct vty *vty, const struct lyd_node *dnode, bool show_defaults)
{
	...
	vty_out(vty, "!\n");
	vty_out(vty, "router isis %s", yang_dnode_get_string(dnode, "area-tag"));
	   ...
	   "Routing process tag\n")
{
	...
	if (strmatch(ip, "ipv6")) {
		if (!yang_dnode_get_bool(dnode, "ipv4-routing"))
			nb_cli_enqueue_change(vty, "./frr-isisd:isis", NB_OP_DESTROY, NULL);
		else
			nb_cli_enqueue_change(vty, "./frr-isisd:isis/ipv6-routing", NB_OP_MODIFY,
					      "false");
	} else {
		if (!yang_dnode_get_bool(dnode, "ipv6-routing"))
			nb_cli_enqueue_change(vty, "./frr-isisd:isis", NB_OP_DESTROY, NULL);
		else
			nb_cli_enqueue_change(vty, "./frr-isisd:isis/ipv4-routing", NB_OP_MODIFY,
					      "false");
	}
      ...
      "Enable BFD support\n")
{
	...
	nb_cli_enqueue_change(vty, "./frr-isisd:isis/bfd-monitoring/enabled", NB_OP_MODIFY,
			      no ? "false" : "true");
      ...
      "Profile name\n")
{
	...
	if (no)
		...
		nb_cli_enqueue_change(vty, "./frr-isisd:isis/bfd-monitoring/profile", NB_OP_MODIFY,
				      profile);
      ...
      "Act as an area router only\n")
{
	nb_cli_enqueue_change(vty, "./is-type", NB_OP_MODIFY,
			      strmatch(level, "level-2-only") ? "level-2" : level);
...
DEFPY_YANG(dynamic_hostname, dynamic_hostname_cmd, "[no] hostname dynamic",
      ...
      "Dynamic hostname\n")
{
	nb_cli_enqueue_change(vty, "./dynamic-hostname", NB_OP_MODIFY, no ? "false" : "true");
...
{
	nb_cli_enqueue_change(vty, "./overload/enabled", NB_OP_MODIFY, no ? "false" : "true");
...
{
	nb_cli_enqueue_change(vty, "./overload/on-startup", NB_OP_MODIFY, val_str);
...
DEFPY_YANG(no_set_overload_bit_on_startup, no_set_overload_bit_on_startup_cmd,
	   ...
	   "Set overload time in seconds\n")
{
	nb_cli_enqueue_change(vty, "./overload/on-startup", NB_OP_DESTROY, NULL);
...
{
	nb_cli_enqueue_change(vty, "./advertise-high-metrics", NB_OP_MODIFY, no ? "false" : "true");
...
{
	nb_cli_enqueue_change(vty, "./attach-send", NB_OP_MODIFY, no ? "false" : "true");
...
{
	nb_cli_enqueue_change(vty, "./attach-receive-ignore", NB_OP_MODIFY, no ? "false" : "true");
      ...
      "Send and check PDUs on receiving\n")
{
	...
	nb_cli_enqueue_change(vty, "./area-password/password", NB_OP_MODIFY, pwd);
	nb_cli_enqueue_change(vty, "./area-password/password-type", NB_OP_MODIFY, pwd_type);
	nb_cli_enqueue_change(vty, "./area-password/authenticate-snp", NB_OP_MODIFY,
			      snp ? snp : "none");
...
void cli_show_isis_area_pwd(struct vty *vty, const struct lyd_node *dnode, bool show_defaults)
{
	...
	vty_out(vty, " area-password %s %s", yang_dnode_get_string(dnode, "password-type"),
      ...
      "Send and check PDUs on receiving\n")
{
	...
	nb_cli_enqueue_change(vty, "./domain-password/password", NB_OP_MODIFY, pwd);
	nb_cli_enqueue_change(vty, "./domain-password/password-type", NB_OP_MODIFY, pwd_type);
	nb_cli_enqueue_change(vty, "./domain-password/authenticate-snp", NB_OP_MODIFY,
			      snp ? snp : "none");
...
void cli_show_isis_domain_pwd(struct vty *vty, const struct lyd_node *dnode, bool show_defaults)
{
	...
	vty_out(vty, " domain-password %s %s", yang_dnode_get_string(dnode, "password-type"),
		yang_dnode_get_string(dnode, "password"));
	snp = yang_dnode_get_string(dnode, "authenticate-snp");
	if (!strmatch("none", snp))
		vty_out(vty, " authenticate snp %s", snp);
      ...
      "Minimum interval in seconds\n")
{
	...
	if (!level || strmatch(level, "level-2"))
		nb_cli_enqueue_change(vty, "./lsp/timers/level-2/generation-interval",
				      NB_OP_MODIFY, val_str);
...
{
	if (!level || strmatch(level, "level-1"))
		nb_cli_enqueue_change(vty, "./lsp/timers/level-1/refresh-interval", NB_OP_MODIFY,
				      val_str);
	if (!level || strmatch(level, "level-2"))
		nb_cli_enqueue_change(vty, "./lsp/timers/level-2/refresh-interval", NB_OP_MODIFY,
				      val_str);
...
{
	if (!level || strmatch(level, "level-1"))
		nb_cli_enqueue_change(vty, "./lsp/timers/level-1/maximum-lifetime", NB_OP_MODIFY,
				      val_str);
	if (!level || strmatch(level, "level-2"))
		nb_cli_enqueue_change(vty, "./lsp/timers/level-2/maximum-lifetime", NB_OP_MODIFY,
				      val_str);
...
{
	const char *l1_refresh = yang_dnode_get_string(dnode, "level-1/refresh-interval");
	const char *l2_refresh = yang_dnode_get_string(dnode, "level-2/refresh-interval");
	const char *l1_lifetime = yang_dnode_get_string(dnode, "level-1/maximum-lifetime");
	const char *l2_lifetime = yang_dnode_get_string(dnode, "level-2/maximum-lifetime");
	const char *l1_gen = yang_dnode_get_string(dnode, "level-1/generation-interval");
	const char *l2_gen = yang_dnode_get_string(dnode, "level-2/generation-interval");

	if (strmatch(l1_refresh, l2_refresh) && strmatch(l1_lifetime, l2_lifetime) &&
	    strmatch(l1_gen, l2_gen))
		vty_out(vty, " lsp-timers gen-interval %s refresh-interval %s max-lifetime %s\n",
...
{
	nb_cli_enqueue_change(vty, "./advertise-passive-only", NB_OP_MODIFY, no ? "false" : "true");
...
{
	if (!level || strmatch(level, "level-1"))
		nb_cli_enqueue_change(vty, "./spf/minimum-interval/level-1", NB_OP_MODIFY, val_str);
	if (!level || strmatch(level, "level-2"))
		nb_cli_enqueue_change(vty, "./spf/minimum-interval/level-2", NB_OP_MODIFY, val_str);
      ...
      "Minimum interval between consecutive SPFs in seconds\n")
{
	...
	if (!level || strmatch(level, "level-2"))
		nb_cli_enqueue_change(vty, "./spf/minimum-interval/level-2", NB_OP_MODIFY, NULL);
...
{
	nb_cli_enqueue_change(vty, "./spf/ietf-backoff-delay", NB_OP_CREATE, NULL);
	nb_cli_enqueue_change(vty, "./spf/ietf-backoff-delay/init-delay", NB_OP_MODIFY,
			      init_delay_str);
	nb_cli_enqueue_change(vty, "./spf/ietf-backoff-delay/short-delay", NB_OP_MODIFY,
			      short_delay_str);
	nb_cli_enqueue_change(vty, "./spf/ietf-backoff-delay/long-delay", NB_OP_MODIFY,
			      long_delay_str);
	nb_cli_enqueue_change(vty, "./spf/ietf-backoff-delay/hold-down", NB_OP_MODIFY,
			      holddown_str);
	nb_cli_enqueue_change(vty, "./spf/ietf-backoff-delay/time-to-learn", NB_OP_MODIFY,
			      time_to_learn_str);
...
DEFPY_YANG(no_spf_delay_ietf, no_spf_delay_ietf_cmd,
      ...
      "Maximum duration needed to learn all the events related to a single failure (in milliseconds)\n")
{
	nb_cli_enqueue_change(vty, "./spf/ietf-backoff-delay", NB_OP_DESTROY, NULL);
      ...
      "Access-list name\n")
{
	...
	snprintf(xpath, XPATH_MAXLEN, "./spf/prefix-priorities/%s/access-list-name", priority);
...
{
	nb_cli_enqueue_change(vty, "./purge-originator", NB_OP_MODIFY, no ? "false" : "true");
...
{
	nb_cli_enqueue_change(vty, "./admin-group-send-zero", NB_OP_MODIFY, no ? "false" : "true");
...
{
	nb_cli_enqueue_change(vty, "./asla-legacy-flag", NB_OP_MODIFY, no ? "false" : "true");
...
{
	nb_cli_enqueue_change(vty, "./mpls-te", NB_OP_CREATE, NULL);
...
DEFPY_YANG(no_isis_mpls_te_on, no_isis_mpls_te_on_cmd, "no mpls-te [on]",
      ...
      "Disable the MPLS-TE functionality\n")
{
	nb_cli_enqueue_change(vty, "./mpls-te", NB_OP_DESTROY, NULL);
...
{
	nb_cli_enqueue_change(vty, "./mpls-te/router-address", NB_OP_MODIFY, router_address_str);
...
DEFPY_YANG(no_isis_mpls_te_router_addr, no_isis_mpls_te_router_addr_cmd,
      ...
      "MPLS-TE router address in IPv4 address format\n")
{
	nb_cli_enqueue_change(vty, "./mpls-te/router-address", NB_OP_DESTROY, NULL);
...
{
	nb_cli_enqueue_change(vty, "./mpls-te/router-address-v6", NB_OP_MODIFY, ipv6_str);
...
DEFPY_YANG(no_isis_mpls_te_router_addr_v6, no_isis_mpls_te_router_addr_v6_cmd,
      ...
      "MPLS-TE router address in IPv6 address format\n")
{
	nb_cli_enqueue_change(vty, "./mpls-te/router-address-v6", NB_OP_DESTROY, NULL);
...
void cli_show_isis_mpls_te_export(struct vty *vty, const struct lyd_node *dnode, bool show_defaults)
{
	if (!yang_dnode_get_bool(dnode, NULL))
		vty_out(vty, " no");
      ...
      "Pointer to route-map entries\n")
{
	if (no)
		...
		nb_cli_enqueue_change(vty, ".", NB_OP_CREATE, NULL);
		nb_cli_enqueue_change(vty, "./always", NB_OP_MODIFY, always ? "true" : "false");
		nb_cli_enqueue_change(vty, "./route-map", rmap ? NB_OP_MODIFY : NB_OP_DESTROY,
				      rmap ? rmap : NULL);
	...
	return nb_cli_apply_changes(vty, "./default-information-originate/%s[level='%s']", ip,
				    level);
}

static void vty_print_def_origin(struct vty *vty, const struct lyd_node *dnode, const char *family,
				 const char *level, bool show_defaults)
{
	...
	if (show_defaults || !yang_dnode_is_default(dnode, "metric"))
		vty_out(vty, " metric %s", yang_dnode_get_string(dnode, "metric"));
      ...
      "Pointer to route-map entries\n")
{
	...
	return nb_cli_apply_changes(vty, "./redistribute/%s[protocol='%s'][level='%s']", ip, proto,
				    level);
}
			     ...
			     "Pointer to route-map entries\n")
{
	...
	if (no) {
		...
		nb_cli_enqueue_change(vty, xpath, NB_OP_DESTROY, NULL);
		rv = nb_cli_apply_changes(vty, "./redistribute/%s[protocol='table'][level='%s']",
					  ip, level);
		if (rv == CMD_SUCCESS) {
			if (isis_redist_table_get_first(vty, &rtda) > 0)
				return CMD_SUCCESS;
			nb_cli_enqueue_change(vty, "./table", NB_OP_DESTROY, NULL);
			nb_cli_apply_changes(vty, "./redistribute/%s[protocol='table'][level='%s']",
					     ip, level);
		}
		return CMD_SUCCESS;
	}
	...
	nb_cli_enqueue_change(vty, xpath_entry, NB_OP_MODIFY, metric_str ? metric_str : NULL);
	return nb_cli_apply_changes(vty, "./redistribute/%s[protocol='table'][level='%s']", ip,
				    level);
}

static void vty_print_redistribute(struct vty *vty, const struct lyd_node *dnode,
				   bool show_defaults, const char *family, bool table)
{
	const char *level;
	...
	"Set overload bit for topology\n")
{
	 ...
	 */
	if (strmatch(topology, "standard") || strmatch(topology, "ipv4-unicast")) {
		vty_out(vty, "Cannot configure IPv4 unicast (Standard) topology\n");
		return CMD_WARNING_CONFIG_FAILED;
	}

	if (strmatch(topology, "ipv4-mgmt"))
		snprintf(base_xpath, XPATH_MAXLEN, "./multi-topology/ipv4-management");
	else if (strmatch(topology, "ipv6-mgmt"))
		snprintf(base_xpath, XPATH_MAXLEN, "./multi-topology/ipv6-management");
	else
		snprintf(base_xpath, XPATH_MAXLEN, "./multi-topology/%s", topology);

	if (no)
		nb_cli_enqueue_change(vty, ".", NB_OP_DESTROY, NULL);
	else {
		nb_cli_enqueue_change(vty, ".", NB_OP_CREATE, NULL);
		nb_cli_enqueue_change(vty, "./overload", NB_OP_MODIFY, overload ? "true" : "false");
	}
...
void cli_show_isis_mt_ipv6_dstsrc(struct vty *vty, const struct lyd_node *dnode, bool show_defaults)
{
...
{
	nb_cli_enqueue_change(vty, "./segment-routing/enabled", NB_OP_MODIFY, "true");
...
{
	nb_cli_enqueue_change(vty, "./segment-routing/enabled", NB_OP_MODIFY, "false");
	...
	"The upper bound of the local block (block size may not exceed 65535)\n")
{
	nb_cli_enqueue_change(vty, "./segment-routing/label-blocks/srgb/lower-bound", NB_OP_MODIFY,
			      gb_lower_bound_str);
	nb_cli_enqueue_change(vty, "./segment-routing/label-blocks/srgb/upper-bound", NB_OP_MODIFY,
			      gb_upper_bound_str);

	nb_cli_enqueue_change(vty, "./segment-routing/label-blocks/srlb/lower-bound", NB_OP_MODIFY,
			      lb_lower_bound ? lb_lower_bound_str : NULL);
	nb_cli_enqueue_change(vty, "./segment-routing/label-blocks/srlb/upper-bound", NB_OP_MODIFY,
			      lb_upper_bound ? lb_upper_bound_str : NULL);
...
void cli_show_isis_label_blocks(struct vty *vty, const struct lyd_node *dnode, bool show_defaults)
{
	...
	if (!yang_dnode_is_default(dnode, "srlb/lower-bound") ||
		...
		vty_out(vty, " local-block %s %s", yang_dnode_get_string(dnode, "srlb/lower-bound"),
...
{
	nb_cli_enqueue_change(vty, "./segment-routing/msd/node-msd", NB_OP_MODIFY, msd_str);
...
DEFPY_YANG (no_isis_sr_node_msd,
       ...
       "Maximum number of label that can be stack (1-16)\n")
{
	nb_cli_enqueue_change(vty, "./segment-routing/msd/node-msd", NB_OP_DESTROY, NULL);
       ...
       "Not a node SID\n")
{
	...
	if (lh_behavior) {
		...
		nb_cli_enqueue_change(vty, "./last-hop-behavior", NB_OP_MODIFY, value);
	} else
		nb_cli_enqueue_change(vty, "./last-hop-behavior", NB_OP_MODIFY, NULL);
	nb_cli_enqueue_change(vty, "./n-flag-clear", NB_OP_MODIFY, n_flag_clear ? "true" : "false");

	return nb_cli_apply_changes(vty, "./segment-routing/prefix-sid-map/prefix-sid[prefix='%s']",
				    prefix_str);
}
       ...
       "Segment Routing over IPv6 (SRv6) Maximum SRv6 SID Depths\n")
{
	...
	if (no) {
		nb_cli_enqueue_change(vty, "./msd/node_msd/max-segs-left", NB_OP_DESTROY, NULL);
		nb_cli_enqueue_change(vty, "./msd/node_msd/end-pop", NB_OP_DESTROY, NULL);
		nb_cli_enqueue_change(vty, "./msd/node_msd/h-encaps", NB_OP_DESTROY, NULL);
		nb_cli_enqueue_change(vty, "./msd/node_msd/end-d", NB_OP_DESTROY, NULL);
		ret = nb_cli_apply_changes(vty, NULL);
	} else
...
{
	if (no)
		nb_cli_enqueue_change(vty, "./max-segs-left", NB_OP_DESTROY, NULL);
	else
		nb_cli_enqueue_change(vty, "./max-segs-left", NB_OP_MODIFY, max_segs_left_str);
       ...
       "Specify Maximum End Pop MSD\n")
{
	if (no)
		...
		nb_cli_enqueue_change(vty, "./max-end-pop", NB_OP_MODIFY, max_end_pop_str);
...
{
	if (no)
		nb_cli_enqueue_change(vty, "./max-h-encaps", NB_OP_DESTROY, NULL);
	else
		nb_cli_enqueue_change(vty, "./max-h-encaps", NB_OP_MODIFY, max_h_encaps_str);
       ...
       "Specify Maximum End D MSD\n")
{
	if (no)
		...
		nb_cli_enqueue_change(vty, "./max-end-d", NB_OP_MODIFY, max_end_d_str);
...
void cli_show_isis_srv6_interface(struct vty *vty, const struct lyd_node *dnode, bool show_defaults)
{
       ...
       "Set priority-limit for level-2 only\n")
{
	if (!level || strmatch(level, "level-1")) {
		if (no) {
			nb_cli_enqueue_change(vty, "./fast-reroute/level-1/lfa/priority-limit",
					      NB_OP_DESTROY, NULL);
		} else {
			nb_cli_enqueue_change(vty, "./fast-reroute/level-1/lfa/priority-limit",
	...
	if (!level || strmatch(level, "level-2")) {
		if (no) {
			nb_cli_enqueue_change(vty, "./fast-reroute/level-2/lfa/priority-limit",
					      NB_OP_DESTROY, NULL);
		} else {
			nb_cli_enqueue_change(vty, "./fast-reroute/level-2/lfa/priority-limit",
					      NB_OP_CREATE, priority);
				      ...
				      bool show_defaults)
{
	vty_out(vty, " fast-reroute lfa tiebreaker %s index %s %s\n",
		yang_dnode_get_string(dnode, "type"), yang_dnode_get_string(dnode, "index"),
		dnode->parent->parent->schema->name);
}
       ...
       "Disable load sharing for level-2 only\n")
{
	if (!level || strmatch(level, "level-1")) {
		if (no) {
			nb_cli_enqueue_change(vty, "./fast-reroute/level-1/lfa/load-sharing",
					      NB_OP_MODIFY, "true");
		} else {
			nb_cli_enqueue_change(vty, "./fast-reroute/level-1/lfa/load-sharing",
					      NB_OP_MODIFY, "false");
	...
	if (!level || strmatch(level, "level-2")) {
		if (no) {
			nb_cli_enqueue_change(vty, "./fast-reroute/level-2/lfa/load-sharing",
					      NB_OP_MODIFY, "true");
		} else {
			nb_cli_enqueue_change(vty, "./fast-reroute/level-2/lfa/load-sharing",
					      NB_OP_MODIFY, "false");
...
{
	if (!level || strmatch(level, "level-1"))
		nb_cli_enqueue_change(vty, "./fast-reroute/level-1/remote-lfa/prefix-list",
				      NB_OP_MODIFY, plist);
	if (!level || strmatch(level, "level-2"))
		nb_cli_enqueue_change(vty, "./fast-reroute/level-2/remote-lfa/prefix-list",
				      NB_OP_MODIFY, plist);
					...
					bool show_defaults)
{
	vty_out(vty, " fast-reroute remote-lfa prefix-list %s %s\n",
		yang_dnode_get_string(dnode, NULL), dnode->parent->parent->schema->name);
}
...
{
	nb_cli_enqueue_change(vty, "./frr-isisd:isis/passive", NB_OP_MODIFY, no ? "false" : "true");
...
DEFPY_YANG(isis_passwd, isis_passwd_cmd, "isis password <md5|clear>$type WORD$pwd",
      ...
      "Circuit password\n")
{
	nb_cli_enqueue_change(vty, "./frr-isisd:isis/password", NB_OP_CREATE, NULL);
	nb_cli_enqueue_change(vty, "./frr-isisd:isis/password/password", NB_OP_MODIFY, pwd);
	nb_cli_enqueue_change(vty, "./frr-isisd:isis/password/password-type", NB_OP_MODIFY, type);
...
DEFPY_YANG(no_isis_passwd, no_isis_passwd_cmd, "no isis password [<md5|clear> WORD]",
      ...
      "Circuit password\n")
{
	nb_cli_enqueue_change(vty, "./frr-isisd:isis/password", NB_OP_DESTROY, NULL);
...
{
	if (!level || strmatch(level, "level-1"))
		nb_cli_enqueue_change(vty, "./frr-isisd:isis/metric/level-1", NB_OP_MODIFY,
				      met_str);
	if (!level || strmatch(level, "level-2"))
		nb_cli_enqueue_change(vty, "./frr-isisd:isis/metric/level-2", NB_OP_MODIFY,
				      met_str);
      ...
      "Default metric value\n")
{
	...
	if (!level || strmatch(level, "level-2"))
		nb_cli_enqueue_change(vty, "./frr-isisd:isis/metric/level-2", NB_OP_MODIFY, NULL);
...
{
	if (!level || strmatch(level, "level-1"))
		nb_cli_enqueue_change(vty, "./frr-isisd:isis/hello/interval/level-1", NB_OP_MODIFY,
				      intv_str);
	if (!level || strmatch(level, "level-2"))
		nb_cli_enqueue_change(vty, "./frr-isisd:isis/hello/interval/level-2", NB_OP_MODIFY,
				      intv_str);
...
{
	if (!level || strmatch(level, "level-1"))
		nb_cli_enqueue_change(vty, "./frr-isisd:isis/hello/multiplier/level-1",
...
void cli_show_ip_isis_hello_multi(struct vty *vty, const struct lyd_node *dnode, bool show_defaults)
{
	const char *l1 = yang_dnode_get_string(dnode, "level-1");
	const char *l2 = yang_dnode_get_string(dnode, "level-2");
...
{
	nb_cli_enqueue_change(vty, "./frr-isisd:isis/disable-three-way-handshake", NB_OP_MODIFY,
			      no ? "true" : "false");
...
{
	if (no) {
		nb_cli_enqueue_change(vty, "./frr-isisd:isis/hello/padding", NB_OP_MODIFY,
				      "disabled");
	} else {
		nb_cli_enqueue_change(vty, "./frr-isisd:isis/hello/padding", NB_OP_MODIFY,
...
{
	if (!level || strmatch(level, "level-1"))
		nb_cli_enqueue_change(vty, "./frr-isisd:isis/csnp-interval/level-1", NB_OP_MODIFY,
				      intv_str);
	if (!level || strmatch(level, "level-2"))
		nb_cli_enqueue_change(vty, "./frr-isisd:isis/csnp-interval/level-2", NB_OP_MODIFY,
				      intv_str);
...
{
	if (!level || strmatch(level, "level-1"))
		nb_cli_enqueue_change(vty, "./frr-isisd:isis/psnp-interval/level-1", NB_OP_MODIFY,
				      intv_str);
	if (!level || strmatch(level, "level-2"))
		nb_cli_enqueue_change(vty, "./frr-isisd:isis/psnp-interval/level-2", NB_OP_MODIFY,
				      intv_str);
      ...
      "IPv6 dst-src topology\n")
{
	...
	if (strmatch(topology, "ipv4-mgmt"))
		return nb_cli_apply_changes(vty, "./frr-isisd:isis/multi-topology/ipv4-management");
	else if (strmatch(topology, "ipv6-mgmt"))
		return nb_cli_apply_changes(vty, "./frr-isisd:isis/multi-topology/ipv6-management");
	if (strmatch(topology, "ipv4-unicast"))
		return nb_cli_apply_changes(vty, "./frr-isisd:isis/multi-topology/standard");
	else
		return nb_cli_apply_changes(vty, "./frr-isisd:isis/multi-topology/%s", topology);
}

void cli_show_ip_isis_mt_standard(struct vty *vty, const struct lyd_node *dnode, bool show_defaults)
{
	if (!yang_dnode_get_bool(dnode, NULL))
		vty_out(vty, " no");
...
{
	nb_cli_enqueue_change(vty, "./frr-isisd:isis/circuit-type", NB_OP_MODIFY,
...
static int ag_change(struct vty *vty, int argc, struct cmd_token **argv, const char *xpath_base,
		     bool no, int start_idx)
{
	char xpath[XPATH_MAXLEN];

	for (int i = start_idx; i < argc; i++) {
		snprintf(xpath, XPATH_MAXLEN, "%s[.='%s']", xpath_base, argv[i]->arg);
		nb_cli_enqueue_change(vty, xpath, no ? NB_OP_DESTROY : NB_OP_CREATE, NULL);
	}
	return nb_cli_apply_changes(vty, NULL);
}
...
{
	nb_cli_enqueue_change(vty, "./frr-isisd:isis/network-type", NB_OP_MODIFY,
...
{
	if (!level || strmatch(level, "level-1"))
		nb_cli_enqueue_change(vty, "./frr-isisd:isis/priority/level-1", NB_OP_MODIFY,
				      prio_str);
	if (!level || strmatch(level, "level-2"))
		nb_cli_enqueue_change(vty, "./frr-isisd:isis/priority/level-2", NB_OP_MODIFY,
				      prio_str);
      ...
      "Specify priority for level-2 routing\n")
{
	...
	if (!level || strmatch(level, "level-2"))
		nb_cli_enqueue_change(vty, "./frr-isisd:isis/priority/level-2", NB_OP_MODIFY, NULL);
...
void cli_show_ip_isis_frr(struct vty *vty, const struct lyd_node *dnode, bool show_defaults)
{
	...
	l1_node_protection = yang_dnode_get_bool(dnode, "level-1/ti-lfa/node-protection");
	l2_node_protection = yang_dnode_get_bool(dnode, "level-2/ti-lfa/node-protection");
					...
					bool show_defaults)
{
	vty_out(vty, " isis fast-reroute lfa %s exclude interface %s\n",
		dnode->parent->parent->schema->name, yang_dnode_get_string(dnode, NULL));
}
      ...
      "Protect against SRLG failures\n")
{
	if (!level || strmatch(level, "level-1")) {
		if (no) {
			...
			nb_cli_enqueue_change(vty,
					      "./frr-isisd:isis/fast-reroute/level-1/ti-lfa/node-protection",
					      NB_OP_MODIFY, node_protection ? "true" : "false");
			nb_cli_enqueue_change(vty,
					      "./frr-isisd:isis/fast-reroute/level-1/ti-lfa/link-fallback",
...
{
	nb_cli_enqueue_change(vty, "./log-pdu-drops", NB_OP_MODIFY, no ? "false" : "true");
...
DEFPY_YANG(isis_mpls_ldp_sync_holddown, isis_mpls_ldp_sync_holddown_cmd,
      ...
      "Time in seconds\n")
{
	nb_cli_enqueue_change(vty, "./mpls/ldp-sync/holddown", NB_OP_MODIFY, holddown_str);
...
{
	nb_cli_enqueue_change(vty, "./mpls/ldp-sync/holddown", NB_OP_DESTROY, NULL);
      ...
      NO_STR "IS-IS routing protocol\n" MPLS_STR MPLS_LDP_SYNC_STR)
{
	...
	nb_cli_enqueue_change(vty, "./frr-isisd:isis/mpls/ldp-sync", NB_OP_MODIFY,
			      no ? "false" : "true");
      ...
      "Time in seconds\n")
{
	...
	nb_cli_enqueue_change(vty, "./frr-isisd:isis/mpls/holddown", NB_OP_MODIFY, holddown_str);
	      ...
	      NO_MPLS_LDP_SYNC_HOLDDOWN_STR "Time in seconds\n")
{
	...
	nb_cli_enqueue_change(vty, "./frr-isisd:isis/mpls/holddown", NB_OP_DESTROY, NULL);
...
{
	nb_cli_enqueue_change(vty, "./advertise-definition", no ? NB_OP_DESTROY : NB_OP_CREATE,
			      no ? NULL : "true");
	return nb_cli_apply_changes(vty, NULL);
}
...
{
	nb_cli_enqueue_change(vty, "./prefix-metric", no ? NB_OP_DESTROY : NB_OP_CREATE, NULL);
	return nb_cli_apply_changes(vty, NULL);
}
...
{
	nb_cli_enqueue_change(vty, "./dplane-sr-mpls", no ? NB_OP_DESTROY : NB_OP_CREATE, NULL);
	return nb_cli_apply_changes(vty, NULL);
}
...
{
	nb_cli_enqueue_change(vty, "./dplane-srv6", no ? NB_OP_DESTROY : NB_OP_CREATE, NULL);
	return nb_cli_apply_changes(vty, NULL);
}
...
{
	nb_cli_enqueue_change(vty, "./dplane-ip", no ? NB_OP_DESTROY : NB_OP_CREATE, NULL);
	return nb_cli_apply_changes(vty, NULL);
}
	   ...
	   "Use Traffic Engineering metric\n")
{
	...
	nb_cli_enqueue_change(vty, "./metric-type", no ? NB_OP_DESTROY : NB_OP_MODIFY,
			      no ? NULL : type);
	return nb_cli_apply_changes(vty, NULL);
}
...
DEFPY_YANG(priority, priority_cmd, "[no] priority (0-255)$priority",
	   ...
	   "Priority value\n")
{
	nb_cli_enqueue_change(vty, "./priority", no ? NB_OP_DESTROY : NB_OP_MODIFY,
...
void cli_show_isis_flex_algo(struct vty *vty, const struct lyd_node *dnode, bool show_defaults)
{
	...
	if (yang_dnode_exists(dnode, "./affinity-include-alls/affinity-include-all")) {
		vty_out(vty, "  affinity include-all");
		yang_dnode_iterate(ag_iter_cb, vty, dnode,
				   "./affinity-include-alls/affinity-include-all");
		vty_out(vty, "\n");
	}

	if (yang_dnode_exists(dnode, "./affinity-include-anies/affinity-include-any")) {
	...
	if (yang_dnode_exists(dnode, "./affinity-exclude-anies/affinity-exclude-any")) {
