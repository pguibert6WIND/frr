...
static void spf_adj_list_parse_lsp(struct isis_spftree *spftree,
				   struct spf_adj_list_head *adj_list, struct isis_lsp *lsp,
				   const uint8_t *pseudo_nodeid, uint32_t pseudo_metric);
...
static void remove_excess_adjs(struct list *adjs)
{
	...
	for (ALL_LIST_ELEMENTS_RO(adjs, node, vadj)) {
		...
		comp = memcmp(candidate_adj->sysid, adj->sysid, ISIS_SYS_ID_LEN);
		if (comp > 0) {
			excess = node;
			continue;
		}
...
const char *vid2string(const struct isis_vertex *vertex, char *buff, int size)
{
	...
	if (VTYPE_IP(vertex->type)) {
		srcdest2str(&vertex->N.ip.p.dest, &vertex->N.ip.p.src, buff, size);
		return buff;
	}
...
static bool prefix_sid_cmp(const void *value1, const void *value2)
{
	...
	if (CHECK_FLAG(c1->N.ip.sr.sid.flags, ISIS_PREFIX_SID_VALUE | ISIS_PREFIX_SID_LOCAL) !=
	    CHECK_FLAG(c2->N.ip.sr.sid.flags, ISIS_PREFIX_SID_VALUE | ISIS_PREFIX_SID_LOCAL))
		return false;
...
static struct isis_vertex *isis_vertex_new(struct isis_spftree *spftree, void *id,
					   enum vertextype vtype)
{
	...
	if (CHECK_FLAG(spftree->flags, F_SPFTREE_HOPCOUNT_METRIC)) {
		vertex->firsthops = hash_create(isis_vertex_queue_hash_key,
						isis_vertex_queue_hash_cmp, NULL);
	}
					    ...
					    struct isis_prefix_sid *psid, bool last_hop)
{
	...
	if (spftree->area->srdb.enabled && psid) {
		if (vertex->N.ip.sr.present && vertex->N.ip.sr.sid.value != psid->value)
			zlog_warn("ISIS-SPF: ignoring different Prefix-SID for route %pFX",
				  &vertex->N.ip.p.dest);
		else {
			vadj->sr.sid = *psid;
			vadj->sr.label = sr_prefix_out_label(spftree->lspdb,
							     vertex->N.ip.p.dest.family, psid,
							     sadj->id, last_hop);
			    ...
			    const struct isis_spf_adj *sadj)
{
	...
	for (ALL_LIST_ELEMENTS_RO(vertex->Adj_N, node, tmp)) {
		if (CHECK_FLAG(spftree->flags, F_SPFTREE_NO_ADJACENCIES)) {
			if (memcmp(sadj->id, tmp->sadj->id, sizeof(sadj->id)) == 0)
				return true;
		} else {
...
static void _isis_spftree_init(struct isis_spftree *tree)
{
	...
	tree->route_table_backup->info = isis_route_table_info_alloc(tree->algorithm);
...
struct isis_spftree *isis_spftree_new(struct isis_area *area, struct lspdb_head *lspdb,
...
static void isis_spftree_adj_del(struct isis_spftree *spftree, struct isis_adjacency *adj)
{
	struct listnode *node;
	...
	for (int tree = SPFTREE_IPV4; tree < SPFTREE_COUNT; tree++) {
		for (int level = ISIS_LEVEL1; level <= ISIS_LEVEL2; level++) {
			...
			area->spftree[tree][level - 1] =
				isis_spftree_new(area, &area->lspdb[level - 1], area->isis->sysid,
						 level, tree, SPF_TYPE_FORWARD, 0,
						 SR_ALGORITHM_SPF);
		}
	}
}
...
static int spf_adj_state_change(struct isis_adjacency *adj)
{
	...
	for (int tree = SPFTREE_IPV4; tree < SPFTREE_COUNT; tree++) {
		for (int level = ISIS_LEVEL1; level <= ISIS_LEVEL2; level++) {
			...
			if (!area->spftree[tree][level - 1])
				continue;
			isis_spftree_adj_del(area->spftree[tree][level - 1], adj);
		}
	}
 ...
 */
struct isis_lsp *isis_root_system_lsp(struct lspdb_head *lspdb, const uint8_t *sysid)
{
	struct isis_lsp *lsp;
...
static struct isis_vertex *isis_spf_add_root(struct isis_spftree *spftree)
{
	...
	if (IS_DEBUG_SPF_EVENTS)
		zlog_debug("ISIS-SPF: A:%hhu added this IS %s %s depth %d dist %d to PATHS",
			   spftree->algorithm, vtype2string(vertex->type),
			   vid2string(vertex, buff, sizeof(buff)), vertex->depth, vertex->d_N);
#endif /* EXTREME_DEBUG */
...
static void vertex_update_firsthops(struct isis_vertex *vertex, struct isis_vertex *parent)
{
	if (vertex->d_N <= 2)
 ...
 */
static struct isis_vertex *isis_spf_add2tent(struct isis_spftree *spftree, enum vertextype vtype,
					     ...
					     struct isis_vertex *parent)
{
	...
	if (vertex != NULL) {
		zlog_err("%s: vertex %s of type %s already in PATH; check for sysId collisions with established neighbors",
	...
	if (vertex != NULL) {
		zlog_err("%s: vertex %s of type %s already in TENT; check for sysId collisions with established neighbors",
			 __func__, vid2string(vertex, buff, sizeof(buff)),
			 vtype2string(vertex->type));
		return NULL;
	}
	...
	if (VTYPE_IP(vtype) && spftree->area->srdb.enabled && psid) {
		...
		if (vertex_psid && !prefix_same(&vertex_psid->N.ip.p.dest, &vertex->N.ip.p.dest)) {
			...
			vertex->N.ip.sr.label = sr_prefix_in_label(area, psid, local);
			...
			(void)hash_get(spftree->prefix_sids, vertex, hash_alloc_intern);
		}
	}
	...
	if (parent && parent->Adj_N && listcount(parent->Adj_N) > 0) {
		...
		for (ALL_LIST_ELEMENTS_RO(parent->Adj_N, node, parent_vadj))
			isis_vertex_adj_add(spftree, vertex, vertex->Adj_N, parent_vadj->sadj,
					    psid, last_hop);
	} else if (sadj) {
		isis_vertex_adj_add(spftree, vertex, vertex->Adj_N, sadj, psid, last_hop);
	}

#ifdef EXTREME_DEBUG
	if (IS_DEBUG_SPF_EVENTS)
		zlog_debug("ISIS-SPF: A:%hhu add to TENT %s %s %s depth %d dist %d adjcount %d",
		      ...
		      uint16_t depth, struct isis_prefix_sid *psid, struct isis_vertex *parent)
{
	...
	if (CHECK_FLAG(spftree->flags, F_SPFTREE_HOPCOUNT_METRIC) && !VTYPE_IS(vtype))
		return;
...
#ifdef EXTREME_DEBUG
		if (IS_DEBUG_SPF_EVENTS)
			zlog_debug("ISIS-SPF: A:%hhu process_N %s %s %s dist %d already found from PATH",
				   spftree->algorithm, print_sys_hostname(vertex->N.id),
				   vtype2string(vtype), vid2string(vertex, buff, sizeof(buff)),
				   dist);
#endif /* EXTREME_DEBUG */
...
/*        1) */
#ifdef EXTREME_DEBUG
		if (IS_DEBUG_SPF_EVENTS)
			zlog_debug("ISIS-SPF: A:%hhu process_N %s %s %s dist %d parent %s adjcount %d",
				   spftree->algorithm, print_sys_hostname(vertex->N.id),
				   vtype2string(vtype), vid2string(vertex, buff, sizeof(buff)),
				   dist, (parent ? print_sys_hostname(parent->N.id) : "null"),
		...
		if (vertex->d_N == dist) {
			...
			struct isis_vertex_adj *parent_vadj;
			for (ALL_LIST_ELEMENTS_RO(parent->Adj_N, node, parent_vadj))
				if (!isis_vertex_adj_exists(spftree, vertex, parent_vadj->sadj)) {
					bool last_hop = (vertex->depth == 2);
...
#ifdef EXTREME_DEBUG
	if (IS_DEBUG_SPF_EVENTS)
		zlog_debug("ISIS-SPF: A:%hhu process_N add2tent %s %s dist %d parent %s",
			   spftree->algorithm, print_sys_hostname(id), vtype2string(vtype), dist,
 ...
 * C.2.6 Step 1
 */
static int isis_spf_process_lsp(struct isis_spftree *spftree, struct isis_lsp *lsp, uint32_t cost,
				uint16_t depth, uint8_t *root_sysid, struct isis_vertex *parent)
{
...
lspfragloop:
...
#ifdef EXTREME_DEBUG
	if (IS_DEBUG_SPF_EVENTS)
		zlog_debug("ISIS-SPF: A:%hhu process_lsp %s", spftree->algorithm,
			   print_sys_hostname(lsp->hdr.lsp_id));
#endif /* EXTREME_DEBUG */
	...
	if (no_overload) {
		if ((pseudo_lsp || spftree->mtid == ISIS_MT_IPV4_UNICAST) &&
			...
			for (r = (struct isis_oldstyle_reach *)lsp->tlvs->oldstyle_reach.head; r;
			     r = r->next) {
				if (fabricd)
					continue;
				...
				if (!LSP_PSEUDO_ID(r->id) &&
				    !memcmp(r->id, root_sysid, ISIS_SYS_ID_LEN))
					continue;
			...
			for (er = te_neighs ? (struct isis_extended_reach *)te_neighs->head : NULL;
				...
				if (flex_algo_id_valid(spftree->algorithm) &&
				    (!sr_algorithm_participated(lsp, spftree->algorithm) ||
				     isis_flex_algo_constraint_drop(spftree, lsp, er)))
					continue;
#endif /* ifndef FABRICD */
	...
	if (!fabricd && !pseudo_lsp && spftree->family == AF_INET &&
	    spftree->mtid == ISIS_MT_IPV4_UNICAST && spftree->area->oldmetric) {
	...
	if (!pseudo_lsp && spftree->family == AF_INET) {
		...
		for (r = ipv4_reachs ? (struct isis_extended_ip_reach *)ipv4_reachs->head : NULL;
			...
			if (spftree->area->srdb.enabled && r->subtlvs) {
				for (struct isis_item *i = r->subtlvs->prefix_sids.head; i;
				     i = i->next) {
	...
	if (!pseudo_lsp && spftree->family == AF_INET6) {
		...
		if (spftree->mtid == ISIS_MT_IPV4_UNICAST)
			...
			ipv6_reachs = isis_lookup_mt_items(&lsp->tlvs->mt_ipv6_reach,
							   spftree->mtid);

		struct isis_ipv6_reach *r;
		for (r = ipv6_reachs ? (struct isis_ipv6_reach *)ipv6_reachs->head : NULL; r;
		     r = r->next) {
			dist = cost + r->metric;
			...
			if (spftree->area->srdb.enabled && r->subtlvs &&
			    r->subtlvs->source_prefix && r->subtlvs->source_prefix->prefixlen) {
				if (spftree->tree_id != SPFTREE_DSTSRC) {
					char buff[VID2STR_BUFFER];
					zlog_warn("Ignoring dest-src route %s in non dest-src topology",
						  srcdest2str(&ip_info.dest,
							      r->subtlvs->source_prefix, buff,
							      sizeof(buff)));
					continue;
				}
		...
		struct isis_item_list *srv6_locators =
			isis_lookup_mt_items(&lsp->tlvs->srv6_locator, spftree->mtid);

		struct isis_srv6_locator_tlv *loc;
		for (loc = srv6_locators ? (struct isis_srv6_locator_tlv *)srv6_locators->head
					 : NULL;
		     loc; loc = loc->next) {
	...
	if ((lsp->hdr.lsp_bits & LSPBIT_ATT) == LSPBIT_ATT &&
	    !spftree->area->attached_bit_rcv_ignore && (spftree->area->is_type & IS_LEVEL_1) &&
	    !isis_level2_adj_up(spftree->area)) {
		memset(&ip_info, 0, sizeof(ip_info));
		if (IS_DEBUG_RTE_EVENTS)
			zlog_debug("ISIS-Spf (%pLS): add default %s route", lsp->hdr.lsp_id,
				   spftree->family == AF_INET ? "ipv4" : "ipv6");
...
static int isis_spf_preload_tent_ip_reach_cb(const struct prefix *prefix, uint32_t metric,
					     bool external, struct isis_subtlvs *subtlvs, void *arg)
{
	...
	if (spftree->area->srdb.enabled && subtlvs) {
		for (struct isis_item *i = subtlvs->prefix_sids.head; i; i = i->next) {
			struct isis_prefix_sid *psid = (struct isis_prefix_sid *)i;
			...
			has_valid_psid = true;
			isis_spf_add_local(spftree, vtype, &ip_info, NULL, 0, psid, parent);
	...
	if (!has_valid_psid)
		isis_spf_add_local(spftree, vtype, &ip_info, NULL, 0, NULL, parent);

	return LSP_ITER_CONTINUE;
}

static void isis_spf_preload_tent(struct isis_spftree *spftree, uint8_t *root_sysid,
				  struct isis_lsp *root_lsp, struct isis_vertex *parent)
{
	...
	if (!CHECK_FLAG(spftree->flags, F_SPFTREE_HOPCOUNT_METRIC)) {
		...
		isis_lsp_iterate_ip_reach(root_lsp, spftree->family, spftree->mtid,
	...
	for (ALL_LIST_ELEMENTS_RO(spftree->sadj_list, node, sadj)) {
		...
		if (isis_lfa_excise_adj_check(spftree, adj_id)) {
			if (IS_DEBUG_LFA)
				zlog_debug("ISIS-SPF: excising adjacency %pPN", sadj->id);
			continue;
		}

		metric = CHECK_FLAG(spftree->flags, F_SPFTREE_HOPCOUNT_METRIC) ? 1 : sadj->metric;
		if (!LSP_PSEUDO_ID(sadj->id)) {
			isis_spf_add_local(spftree,
					   CHECK_FLAG(sadj->flags, F_ISIS_SPF_ADJ_OLDMETRIC)
						   ? VTYPE_NONPSEUDO_IS
						   : VTYPE_NONPSEUDO_TE_IS,
					   sadj->id, sadj, metric, NULL, parent);
		} else if (sadj->lsp) {
			isis_spf_process_lsp(spftree, sadj->lsp, metric, 0, spftree->sysid, parent);
		}
	}
}
...
static int spf_adj_find_reverse_metric_cb(const uint8_t *id, uint32_t metric, bool oldmetric,
...
static void spf_adj_list_parse_tlv(struct isis_spftree *spftree, struct spf_adj_list_head *adj_list,
				   ...
				   struct isis_ext_subtlvs *subtlvs)
{
	...
	if (lsp == NULL || lsp->hdr.rem_lifetime == 0) {
		zlog_warn("ISIS-SPF: No LSP found from root to L%d %pLS", spftree->level, lspid);
		return;
	}
	...
	if (desig_is_id) {
		memcpy(sadj->lan.desig_is_id, desig_is_id, sizeof(sadj->lan.desig_is_id));
				   ...
				   const uint8_t *pseudo_nodeid, uint32_t pseudo_metric)
{
	...
	if (lsp->tlvs) {
		if (pseudo_lsp || spftree->mtid == ISIS_MT_IPV4_UNICAST) {
			...
			for (struct isis_oldstyle_reach *reach = (struct isis_oldstyle_reach *)head;
			     reach; reach = reach->next) {
				spf_adj_list_parse_tlv(spftree, adj_list, reach->id, pseudo_nodeid,
						       pseudo_metric, reach->metric, true, NULL);
			}
		}
		...
		if (pseudo_lsp || spftree->mtid == ISIS_MT_IPV4_UNICAST)
			...
			te_neighs = isis_get_mt_items(&lsp->tlvs->mt_reach, spftree->mtid);
		if (te_neighs) {
			head = te_neighs->head;
			for (struct isis_extended_reach *reach = (struct isis_extended_reach *)head;
			     reach; reach = reach->next) {
#ifndef FABRICD
				/*
				...
				if (flex_algo_id_valid(spftree->algorithm) &&
				    (!sr_algorithm_participated(lsp, spftree->algorithm) ||
				     isis_flex_algo_constraint_drop(spftree, lsp, reach)))
					continue;
#endif /* ifndef FABRICD */

				spf_adj_list_parse_tlv(spftree, adj_list, reach->id, pseudo_nodeid,
						       pseudo_metric, reach->metric, false,
						       reach->subtlvs);
			}
		}
	}
	...
	for (ALL_LIST_ELEMENTS_RO(lsp->lspu.frags, node, frag)) {
		...
		spf_adj_list_parse_lsp(spftree, adj_list, frag, pseudo_nodeid, pseudo_metric);
	}
}
 ...
 */
static void add_to_paths(struct isis_spftree *spftree, struct isis_vertex *vertex)
{
#ifdef EXTREME_DEBUG
	char buff[VID2STR_BUFFER];
#endif /* EXTREME_DEBUG */
...
#ifdef EXTREME_DEBUG
	if (IS_DEBUG_SPF_EVENTS)
		zlog_debug("ISIS-SPF: A:%hhu S:%p added %s %s %s depth %d dist %d to PATHS",
			   spftree->algorithm, spftree, print_sys_hostname(vertex->N.id),
			   vtype2string(vertex->type), vid2string(vertex, buff, sizeof(buff)),
...
static void init_spt(struct isis_spftree *spftree, int mtid)
{
	...
	memset(&spftree->lfa.protection_counters, 0, sizeof(spftree->lfa.protection_counters));

	spftree->mtid = mtid;
}

static enum spf_prefix_priority spf_prefix_priority(struct isis_spftree *spftree,
						    struct isis_vertex *vertex)
{
	struct isis_area *area = spftree->area;
	struct prefix *prefix = &vertex->N.ip.p.dest;

	for (int priority = SPF_PREFIX_PRIO_CRITICAL; priority <= SPF_PREFIX_PRIO_MEDIUM;
	     priority++) {
...
static void spf_path_process(struct isis_spftree *spftree, struct isis_vertex *vertex)
{
	struct isis_area *area = spftree->area;
	...
	if (VTYPE_IP(vertex->type) && !CHECK_FLAG(spftree->flags, F_SPFTREE_NO_ROUTES)) {
		...
		if (vertex->depth == 1 || listcount(vertex->Adj_N) > 0) {
			...
			case SPF_TYPE_TI_LFA:
				if (priority > area->lfa_priority_limit[level - 1]) {
					if (IS_DEBUG_LFA)
						zlog_debug("ISIS-LFA: skipping %s %s (low prefix priority)",
							   vtype2string(vertex->type),
			...
			case SPF_TYPE_TI_LFA:
				...
				pre_spftree->lfa.protection_counters.tilfa[vertex->N.ip.priority] +=
					1;
				break;
			case SPF_TYPE_FORWARD:
			case SPF_TYPE_REVERSE:
				 ...
				 */
				if (vertex->depth > 1) {
					spftree->lfa.protection_counters.total[priority] += 1;
					if (listcount(vertex->Adj_N) > 1)
						spftree->lfa.protection_counters.ecmp[priority] +=
							1;
				}
				break;
			}
...
#ifdef EXTREME_DEBUG
			zlog_debug("ISIS-SPF: A:%hhu create route pfx %pFX dist %d, sr.algo %d, table %p, rv %p",
				   spftree->algorithm, &vertex->N.ip.p.dest, vertex->d_N,
				   vertex->N.ip.sr.algorithm, route_table, ri);
#endif /* EXTREME_DEBUG */
		} else if (IS_DEBUG_SPF_EVENTS)
			zlog_debug("ISIS-SPF: no adjacencies, do not install route for %s depth %d dist %d",
				   vid2string(vertex, buff, sizeof(buff)), vertex->depth,
				   vertex->d_N);
	}
}

static void isis_spf_loop(struct isis_spftree *spftree, uint8_t *root_sysid)
{
	struct isis_vertex *vertex;
	struct isis_lsp *lsp;
	struct listnode *node;
	...
	while (isis_vertex_queue_count(&spftree->tents)) {
...
#ifdef EXTREME_DEBUG
		zlog_debug("ISIS-SPF: A:%hhu get TENT node %s %s depth %d dist %d to PATHS",
			   spftree->algorithm, print_sys_hostname(vertex->N.id),
		...
		if (!lsp) {
			zlog_warn("ISIS-SPF: No LSP found for %pPN", vertex->N.id);
			continue;
		}

		isis_spf_process_lsp(spftree, lsp, vertex->d_N, vertex->depth, root_sysid, vertex);
	}
	...
	for (ALL_QUEUE_ELEMENTS_RO(&spftree->paths, node, vertex)) {
		...
		case VTYPE_IPREACH_INTERNAL:
		case VTYPE_IPREACH_EXTERNAL:
			if (isis_find_vertex(&spftree->paths, &vertex->N, VTYPE_IPREACH_TE))
				continue;
			break;
		case VTYPE_PSEUDO_IS:
		case VTYPE_PSEUDO_TE_IS:
...
{
	if (!spftree)
		spftree = isis_spftree_new(area, &area->lspdb[IS_LEVEL_2 - 1], sysid, ISIS_LEVEL2,
	...
	if (!memcmp(sysid, area->isis->sysid, ISIS_SYS_ID_LEN)) {
		...
		if (root_lsp) {
			...
			isis_spf_preload_tent(spftree, sysid, root_lsp, root_vertex);
		}
	} else {
		isis_vertex_queue_insert(&spftree->tents,
...
void isis_run_spf(struct isis_spftree *spftree)
{
	...
	root_lsp = isis_root_system_lsp(spftree->lspdb, spftree->sysid);
	if (root_lsp == NULL) {
		zlog_err("ISIS-SPF: could not find own l%d LSP!", spftree->level);
		return;
	}
	...
	case SPFTREE_IPV4:
		...
		break;
	case SPFTREE_IPV6:
		mt_router_info = isis_tlvs_lookup_mt_router_info(root_lsp->tlvs,
	...
	case SPFTREE_DSTSRC:
		...
		break;
	case SPFTREE_COUNT:
		zlog_err("%s should never be called with SPFTREE_COUNT as argument!", __func__);
		exit(1);
	}
	 ...
	 */
	if (flex_algo_id_valid(spftree->algorithm)) {
		flex_algo_enabled = isis_flex_algo_elected_supported(spftree->algorithm,
								     spftree->area);
		if (flex_algo_enabled !=
		    flex_algo_get_state(spftree->area->flex_algos, spftree->algorithm)) {
			/* actual state is inconsistent with local LSP */
			lsp_regenerate_schedule(spftree->area, spftree->area->is_type, 0);
			goto out;
		}
		if (!flex_algo_enabled) {
	 ...
	 * C.2.7 Step 2
	 */
	if (!isis_vertex_queue_count(&spftree->tents) && (IS_DEBUG_SPF_EVENTS)) {
...
out:
	...
	spftree->last_run_duration = ((time_end.tv_sec - time_start.tv_sec) * 1000000) +
				     (time_end.tv_usec - time_start.tv_usec);
}

static void isis_run_spf_with_protection(struct isis_area *area, struct isis_spftree *spftree)
{
...
void isis_spf_verify_routes(struct isis_area *area, struct isis_spftree **trees, int tree)
{
	if (area->is_type == IS_LEVEL_1) {
		isis_route_verify_table(area, trees[0]->route_table, trees[0]->route_table_backup,
					tree);
	} else if (area->is_type == IS_LEVEL_2) {
		isis_route_verify_table(area, trees[1]->route_table, trees[1]->route_table_backup,
					tree);
	} else {
		isis_route_verify_merge(area, trees[0]->route_table, trees[0]->route_table_backup,
					trees[1]->route_table, trees[1]->route_table_backup, tree);
	}
}
...
void isis_spf_switchover_routes(struct isis_area *area, struct isis_spftree **trees, int family,
				union g_addr *nexthop_ip, ifindex_t ifindex, int level)
{
	isis_route_switchover_nexthop(area, trees[level - 1]->route_table, family, nexthop_ip,
				      ifindex);
}
...
static void isis_run_spf_cb(struct event *event)
{
	...
	if (IS_DEBUG_SPF_EVENTS)
		zlog_debug("ISIS-SPF (%s) L%d SPF needed, periodic SPF", area->area_tag, level);

	if (area->ip_circuits) {
		isis_run_spf_with_protection(area, area->spftree[SPFTREE_IPV4][level - 1]);
#ifndef FABRICD
		for (ALL_LIST_ELEMENTS_RO(area->flex_algos->flex_algos, node, fa)) {
			data = fa->data;
			isis_run_spf_with_protection(area, data->spftree[SPFTREE_IPV4][level - 1]);
		}
#endif /* ifndef FABRICD */
		have_run = 1;
	}
	if (area->ipv6_circuits) {
		isis_run_spf_with_protection(area, area->spftree[SPFTREE_IPV6][level - 1]);
#ifndef FABRICD
		for (ALL_LIST_ELEMENTS_RO(area->flex_algos->flex_algos, node, fa)) {
			data = fa->data;
			isis_run_spf_with_protection(area, data->spftree[SPFTREE_IPV6][level - 1]);
		}
#endif /* ifndef FABRICD */
		have_run = 1;
	}
	if (area->ipv6_circuits && isis_area_ipv6_dstsrc_enabled(area)) {
		isis_run_spf_with_protection(area, area->spftree[SPFTREE_DSTSRC][level - 1]);
		have_run = 1;
	}
...
int _isis_spf_schedule(struct isis_area *area, int level, const char *func, const char *file,
		       int line)
{
	struct isis_spftree *spftree;
	time_t now;
	...
	if (IS_DEBUG_SPF_EVENTS) {
		zlog_debug("ISIS-SPF (%s) L%d SPF schedule called, lastrun %ld sec ago Caller: %s %s:%d",
	...
	if (diff >= area->min_spf_interval[level - 1] || area->bfd_force_spf_refresh) {
		/*
		...
		if (area->bfd_force_spf_refresh) {
			zlog_debug("ISIS-SPF (%s) L%d SPF scheduled immediately due to BFD 'down' message",
				   area->area_tag, level);
	...
	event_add_timer(master, isis_run_spf_cb, isis_run_spf_arg(area, level), timer,
			&area->spf_timer[level - 1]);

	if (IS_DEBUG_SPF_EVENTS)
		zlog_debug("ISIS-SPF (%s) L%d SPF scheduled %ld sec from now", area->area_tag,
			   level, timer);

	return ISIS_OK;
}

static void isis_print_paths(struct vty *vty, struct isis_vertex_queue *queue, uint8_t *root_sysid,
			     struct json_object **json)
{
	...
	for (ALL_QUEUE_ELEMENTS_RO(queue, node, vertex)) {
		if (VTYPE_IS(vertex->type) &&
			...
			/* display here */
			ttable_add_row(tt, "%s|%s|%s|%s|%s|%s", print_sys_hostname(root_sysid), "",
				       "", "", "", "");
			continue;
		}
		...
		snprintf(vertex_name, sizeof(vertex_name), "%s",
			 vid2string(vertex, buff, sizeof(buff)));
		snprintf(vertex_typestr, sizeof(vertex_typestr), "%s", vtype2string(vertex->type));
		snprintf(vertex_metricstr, sizeof(vertex_metricstr), "%u", vertex->d_N);
		for (unsigned int i = 0; i < MAX(vertex->Adj_N ? listcount(vertex->Adj_N) : 0,
			...
			if (vadj) {
				...
				snprintf(vertex_nexthop, sizeof(vertex_nexthop), "%s",
					 print_sys_hostname(sadj->id));
				snprintf(vertex_interface, sizeof(vertex_interface), "%s",
					 sadj->adj ? sadj->adj->circuit->interface->name : "-");
			}
			...
			if (pvertex) {
				...
				snprintf(vertex_parent, sizeof(vertex_parent), "%s(%d)",
					 vid2string(pvertex, buff, sizeof(buff)), pvertex->type);
			}

			++rows;
		}
		ttable_add_row(tt, "%s|%s|%s|%s|%s|%s", vertex_name, vertex_typestr,
			       vertex_metricstr, vertex_nexthop, vertex_interface, vertex_parent);
	}
	if (json == NULL) {
		...
		*json = ttable_json_with_json_text(tt, "ssdsss",
						   "vertex|type|metric|nextHop|interface|parent");
	ttable_del(tt);
}

void isis_print_spftree(struct vty *vty, struct isis_spftree *spftree, struct json_object **json)
{
...
static void show_isis_topology_common(struct vty *vty, int levels, struct isis *isis, uint8_t algo,
				      json_object **json)
{
#ifndef FABRICD
	...
	frr_each (isis_area_list, &isis->area_list, area) {
		...
		if (json) {
			jstr = json_object_new_string(area->area_tag ? area->area_tag : "null");
			json_object_object_add(*json, "area", jstr);
			json_object_int_add(*json, "algorithm", algo);
		} else {
			vty_out(vty, "Area %s:", area->area_tag ? area->area_tag : "null");
		...
		for (int level = ISIS_LEVEL1; level <= ISIS_LEVELS; level++) {
			...
			if (json) {
				...
				jstr = json_object_new_string(area->area_tag ? area->area_tag
									     : "null");
...
#ifndef FABRICD
				if (fa_data)
					spftree = fa_data->spftree[SPFTREE_IPV4][level - 1];
				else
#endif /* ifndef FABRICD */
					spftree = area->spftree[SPFTREE_IPV4][level - 1];

				isis_print_spftree(vty, spftree, json ? &json_val : NULL);
				if (json && json_val)
					json_object_object_add(json_level, "ipv4-paths", json_val);
			}
			if (area->ipv6_circuits > 0) {
				json_val = NULL;
#ifndef FABRICD
				if (fa_data)
					spftree = fa_data->spftree[SPFTREE_IPV6][level - 1];
				else
#endif /* ifndef FABRICD */
					spftree = area->spftree[SPFTREE_IPV6][level - 1];
				isis_print_spftree(vty, spftree, json ? &json_val : NULL);
				if (json && json_val)
					json_object_object_add(json_level, "ipv6-paths", json_val);
			}
			if (isis_area_ipv6_dstsrc_enabled(area)) {
				json_val = NULL;
#ifndef FABRICD
				if (fa_data)
					spftree = fa_data->spftree[SPFTREE_DSTSRC][level - 1];
				else
#endif /* ifndef FABRICD */
					spftree = area->spftree[SPFTREE_DSTSRC][level - 1];
				isis_print_spftree(vty, spftree, json ? &json_val : NULL);
				if (json && json_val) {
					json_object_object_add(json_level, "ipv6-dstsrc-paths",
							       json_val);
				}
			}
			if (json) {
		...
		if (fabricd_spftree(area)) {
			...
			vty_out(vty, "IS-IS paths to level-2 routers with hop-by-hop metric\n");
			isis_print_paths(vty, &fabricd_spftree(area)->paths, isis->sysid,
					 json ? &json_val : NULL);
			if (json && json_val)
				json_object_object_add(json_level, "fabricd-paths", json_val);
			else
				vty_out(vty, "\n");
		}
		if (!json)
...
)
{
	...
	if (all_algorithm) {
		...
		show_isis_topology_common(vty, levels, isis, (uint8_t)algorithm,
					  uj ? &json_vrf : NULL);
	if (uj) {
		json_object_object_add(json_vrf, "vrf_id", json_object_new_int(isis->vrf_id));
		json_object_array_add(json, json_vrf);
	}
out:
	if (uj) {
...
#ifndef FABRICD
static void show_isis_flex_algo_display_eag(struct vty *vty, char *buf, int indent,
					    struct admin_group *admin_group)
{
	if (admin_group_zero(admin_group))
		...
		vty_out(vty, "%s\n",
			admin_group_string(buf, ADMIN_GROUP_PRINT_MAX_SIZE, indent, admin_group));
...
static void show_isis_flex_algo_common(struct vty *vty, struct isis *isis, uint8_t algorithm)
{
	...
	frr_each (isis_area_list, &isis->area_list, area) {
		...
		for (algo = 0; algo < SR_ALGORITHM_COUNT; algo++) {
			if (algorithm != SR_ALGORITHM_UNSET && algorithm != algo)
				continue;
			...
			fad_identical = flex_algo_definition_cmp(fa, &router_fad->fad);
			fad_supported = isis_flex_algo_supported(&router_fad->fad);
			vty_out(vty, "  Priority: %d\n", router_fad->fad.priority);
			vty_out(vty, "  Equal to local: %s\n", fad_identical ? "yes" : "no");
			vty_out(vty, "  Local state: %s\n",
				fad_supported ? "enabled" : "disabled (unsupported definition)");
			...
			vty_out(vty, "  Metric type: %s\n",
				flex_algo_metric_type_print(buf, sizeof(buf),
							    router_fad->fad.metric_type));
			vty_out(vty, "  Prefix-metric: %s\n",
				CHECK_FLAG(router_fad->fad.flags, FAD_FLAG_M) ? "enabled"
									      : "disabled");
			if (router_fad->fad.flags != 0 && router_fad->fad.flags != FAD_FLAG_M)
				vty_out(vty, "  Flags: 0x%x\n", router_fad->fad.flags);
			vty_out(vty, "  Exclude SRLG: %s\n",
				router_fad->fad.exclude_srlg ? "enabled" : "disabled");

			admin_group = &router_fad->fad.admin_group_exclude_any;
			indent = vty_out(vty, "  Exclude-any admin-group: ");
			show_isis_flex_algo_display_eag(vty, buf, indent, admin_group);
			...
			if (router_fad->fad.unsupported_subtlv)
				vty_out(vty, "  Unsupported sub-TLV: Present (see logs)");
			     ...
			     bool json)
{
	...
	for (int alg = 0; alg < SR_ALGORITHM_COUNT; alg++) {
		for (ALL_LIST_ELEMENTS_RO(rinfo->sr_algo[alg].nexthops, node, nexthop)) {
			...
			if (prefix_sid) {
				...
				if (rinfo->sr_algo[alg].present) {
					...
					sr_op2str(buf_lblop, sizeof(buf_lblop),
						  rinfo->sr_algo[alg].label, nexthop->sr.label);
				...
				if (first || json) {
					ttable_add_row(tt, "%s|%u|%s|%s|%s|%s|%d", buf_prefix,
						       rinfo->cost, buf_iface, buf_nhop, buf_sid,
						       buf_lblop, alg);
					first = false;
				} else
				...
				if (first || json) {
					...
					ttable_add_row(tt, "||%s|%s|%s", buf_iface, buf_nhop,
						       buf_labels);
			}
		}
	}
	...
	if (list_isempty(rinfo->nexthops)) {
		if (prefix_sid) {
			...
			if (rinfo->sr_algo[SR_ALGORITHM_SPF].present) {
				snprintf(buf_sid, sizeof(buf_sid), "%u",
					 rinfo->sr_algo[SR_ALGORITHM_SPF].sid.value);
				sr_op2str(buf_lblop, sizeof(buf_lblop),
			...
			ttable_add_row(tt, "%s|%u|%s|%s|%s|%s", buf_prefix, rinfo->cost, "-", "-",
				       buf_sid, buf_lblop);
		} else
			ttable_add_row(tt, "%s|%u|%s|%s|%s", buf_prefix, rinfo->cost, "-", "-",
				       "-");
	}
}
		       ...
		       bool prefix_sid, bool backup)
{
	...
	for (rn = route_top(route_table); rn; rn = route_next(rn)) {
		...
		isis_print_route(tt, &rn->p, rinfo, prefix_sid, no_adjacencies, json != NULL);
	}
...
static void show_isis_route_common(struct vty *vty, int levels, struct isis *isis, bool prefix_sid,
				   bool backup, uint8_t algo, json_object **json)
{
	json_object *json_level = NULL, *jstr = NULL, *json_val;
#ifndef FABRICD
	struct isis_flex_algo_data *fa_data;
...
static void show_isis_route_all_algos(struct vty *vty, int levels, struct isis *isis,
				      bool prefix_sid, bool backup, json_object **json)
{
	uint16_t algo;
	...
	for (algo = SR_ALGORITHM_FLEX_MIN; algo <= SR_ALGORITHM_FLEX_MAX; algo++) {
		show_isis_route_common(vty, levels, isis, prefix_sid, backup, (uint8_t)algo,
				       json ? &json_algo : NULL);
		if (!json)
			continue;
		...
		json_object_object_add(json_algo, "algorithm", json_object_new_int(algo));
      ...
      JSON_STR)
{
	...
	if (isis != NULL) {
		if (all_algorithm)
			...
			show_isis_route_common(vty, levels, isis, prefix_sid, backup, algorithm,
					       uj ? &json_vrf : NULL);
		if (uj) {
			json_object_object_add(json_vrf, "vrf_id",
					       json_object_new_int(isis->vrf_id));
...
static void isis_print_frr_summary_line(struct ttable *tt, const char *protection,
					uint32_t counters[SPF_PREFIX_PRIO_MAX])
{
	...
	ttable_add_row(tt, "%s|%u|%u|%u|%u|%u", protection, critical, high, medium, low, total);
}

static void isis_print_frr_summary_line_coverage(struct ttable *tt, const char *protection,
						 double counters[SPF_PREFIX_PRIO_MAX], double total)
{
	...
	ttable_add_row(tt, "%s|%.2f%%|%.2f%%|%.2f%%|%.2f%%|%.2f%%", protection, critical, high,
		       medium, low, total);
}

static void isis_print_frr_summary(struct vty *vty, struct isis_spftree *spftree)
{
	struct ttable *tt;
	char *table;
	const char *tree_id_text = NULL;
	...
	/* Prepare table. */
	tt = ttable_new(&ttable_styles[TTSTYLE_BLANK]);
	ttable_add_row(tt, "Protection \\ Priority|Critical|High    |Medium  |Low     |Total");
	tt->style.cell.rpad = 2;
	tt->style.corner = '+';
	ttable_restyle(tt);
	ttable_rowseps(tt, 0, BOTTOM, true, '-');
	...
	for (int priority = SPF_PREFIX_PRIO_CRITICAL; priority < SPF_PREFIX_PRIO_MAX; priority++) {
		...
		protectd[priority] = lfa[priority] + rlfa[priority] + tilfa[priority] +
				     ecmp[priority];
		...
		if (!total[priority])
			...
			coverage[priority] = protectd[priority] / (double)total[priority];
	}
	...
	/* Add rows. */
	isis_print_frr_summary_line(tt, "Classic LFA", spftree->lfa.protection_counters.lfa);
	isis_print_frr_summary_line(tt, "Remote LFA", spftree->lfa.protection_counters.rlfa);
	isis_print_frr_summary_line(tt, "Topology Independent LFA",
				    spftree->lfa.protection_counters.tilfa);
	isis_print_frr_summary_line(tt, "ECMP", spftree->lfa.protection_counters.ecmp);
	isis_print_frr_summary_line(tt, "Unprotected", unprotected);
	isis_print_frr_summary_line_coverage(tt, "Protection coverage", coverage, coverage_total);
...
static void show_isis_frr_summary_common(struct vty *vty, int levels, struct isis *isis)
{
	...
	frr_each (isis_area_list, &isis->area_list, area) {
		...
		for (int level = ISIS_LEVEL1; level <= ISIS_LEVELS; level++) {
			...
			if (area->ip_circuits > 0)
				isis_print_frr_summary(vty, area->spftree[SPFTREE_IPV4][level - 1]);
			if (area->ipv6_circuits > 0)
				isis_print_frr_summary(vty, area->spftree[SPFTREE_IPV6][level - 1]);
			if (isis_area_ipv6_dstsrc_enabled(area))
				isis_print_frr_summary(vty,
						       area->spftree[SPFTREE_DSTSRC][level - 1]);
		}
	}
}
...
void isis_spf_print_json(struct isis_spftree *spftree, struct json_object *json)
{
	...
	json_object_int_add(json, "last-run-duration-usec", spftree->last_run_duration);
