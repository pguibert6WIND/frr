...
static void isis_route_update(struct isis_area *area, struct prefix *prefix,
			      struct prefix_ipv6 *src_p, struct isis_route_info *route_info);

static struct mpls_label_stack *label_stack_dup(const struct mpls_label_stack *const orig)
{
	...
	copy = XCALLOC(MTYPE_ISIS_NEXTHOP_LABELS, sizeof(struct mpls_label_stack) + array_size);
	copy->num_labels = orig->num_labels;
	memcpy(copy->label, orig->label, array_size);
	return copy;
}

static struct isis_nexthop *isis_nexthop_create(int family, const union g_addr *const ip,
						ifindex_t ifindex)
{
					  ...
					  ifindex_t ifindex)
{
	...
	for (ALL_LIST_ELEMENTS_RO(nexthops, node, nh)) {
		...
		case AF_INET6:
			...
			break;
		default:
			flog_err(EC_LIB_DEVELOPMENT, "%s: unknown address family [%d]", __func__,
				 family);
			exit(1);
		}
	...
	case AF_INET:
		for (unsigned int i = 0; i < adj->ipv4_address_count; i++) {
			...
			if (!nexthoplookup(nexthops, AF_INET, &ip,
				...
				nh = isis_nexthop_create(AF_INET, &ip,
							 adj->circuit->interface->ifindex);
...
static struct isis_route_info *isis_route_info_new(struct prefix *prefix,
						   struct prefix_ipv6 *src_p, uint32_t cost,
						   uint32_t depth, struct isis_sr_psid_info *sr,
						   struct list *adjacencies, bool allow_ecmp)
{
	...
	rinfo->sr_algo[sr->algorithm].nexthops_backup = rinfo->backup ? rinfo->backup->nexthops
								      : NULL;
...
static void isis_route_info_delete(struct isis_route_info *route_info)
{
	for (int i = 0; i < SR_ALGORITHM_COUNT; i++) {
		...
		route_info->sr_algo[i].nexthops->del = (void (*)(void *))isis_nexthop_delete;
...
static int isis_route_info_same(struct isis_route_info *new, struct isis_route_info *old,
				char *buf, size_t buf_size)
{
	...
	if (new->cost != old->cost) {
		if (buf)
			snprintf(buf, buf_size, "cost (old: %u, new: %u)", old->cost, new->cost);
		return 0;
	}

	if (new->depth != old->depth) {
		if (buf)
			snprintf(buf, buf_size, "depth (old: %u, new: %u)", old->depth, new->depth);
		return 0;
	}
	...
	for (ALL_LIST_ELEMENTS_RO(new->nexthops, node, new_nh)) {
		old_nh = nexthoplookup(old->nexthops, new_nh->family, &new_nh->ip, new_nh->ifindex);
		if (!old_nh) {
			if (buf)
					  ...
					  struct route_table *table)
{
	...
	rinfo_new = isis_route_info_new(prefix, src_p, cost, depth, sr, adjacencies, allow_ecmp);
	route_node = srcdest_rnode_get(table, prefix, src_p);

	rinfo_old = route_node->info;
	if (!rinfo_old) {
		if (IS_DEBUG_RTE_EVENTS)
			zlog_debug("ISIS-Rte (%s) route created: %pFX", area->area_tag, prefix);
...
#ifdef EXTREME_DEBUG
		if (IS_DEBUG_RTE_EVENTS)
			zlog_debug("ISIS-Rte (%s) route already exists: %pFX", area->area_tag,
				   prefix);
#endif /* EXTREME_DEBUG */
		if (isis_route_info_same(rinfo_new, rinfo_old, change_buf, sizeof(change_buf))) {
#ifdef EXTREME_DEBUG
			if (IS_DEBUG_RTE_EVENTS)
				zlog_debug("ISIS-Rte (%s) route unchanged: %pFX", area->area_tag,
					   prefix);
#endif /* EXTREME_DEBUG */
			...
			if (IS_DEBUG_RTE_EVENTS)
				zlog_debug("ISIS-Rte (%s): route changed: %pFX, change: %s",
					   area->area_tag, prefix, change_buf);
			for (int i = 0; i < SR_ALGORITHM_COUNT; i++)
				rinfo_new->sr_algo_previous[i] = rinfo_old->sr_algo[i];
...
void isis_route_delete(struct isis_area *area, struct route_node *rode, struct route_table *table)
{
	struct isis_route_info *rinfo;
	...
	if (rinfo == NULL) {
		if (IS_DEBUG_RTE_EVENTS)
			zlog_debug("ISIS-Rte: tried to delete non-existent route %s", buff);
		return;
	}
...
static void isis_route_remove_previous_sid(struct isis_area *area, struct prefix *prefix,
					   struct isis_route_info *route_info)
{
	/*
					 ...
					 struct isis_route_info *rinfo)
{
	for (int i = 0; i < SR_ALGORITHM_COUNT; i++) {
		if (rinfo->sr_algo[i].present) {
			...
			assert(rinfo->backup ? rinfo->backup->nexthops != NULL : true);
			...
			mrinfo->sr_algo[i].nexthops =
				isis_nexthop_list_dup(rinfo->sr_algo[i].nexthops);
		}
	}
			      ...
			      struct prefix_ipv6 *src_p, struct isis_route_info *route_info)
{
	...
	if (CHECK_FLAG(route_info->flag, ISIS_ROUTE_FLAG_ACTIVE)) {
		...
		for (int i = 0; i < SR_ALGORITHM_COUNT; i++) {
			 ...
			 */
			if (sr_algo.present)
				isis_zebra_prefix_sid_install(area, prefix, &sr_algo);
...
static void _isis_route_verify_table(struct isis_area *area, struct route_table *table,
				     struct route_table *table_backup, struct route_table **tables)
{
	...
	for (rnode = route_top(table); rnode; rnode = srcdest_route_next(rnode)) {
		...
		srcdest_rnode_prefixes(rnode, (const struct prefix **)&dst_p,
				       (const struct prefix **)&src_p);
		...
		if (table_backup) {
			...
			rnode_bck = srcdest_rnode_lookup(table_backup, dst_p, src_p);
			if (rnode_bck) {
				rinfo->backup = rnode_bck->info;
				rinfo->sr_algo[algorithm].nexthops_backup = rinfo->backup->nexthops;
				UNSET_FLAG(rinfo->flag, ISIS_ROUTE_FLAG_ZEBRA_SYNCED);
		...
		if (IS_DEBUG_RTE_EVENTS) {
			...
			zlog_debug("ISIS-Rte (%s): route validate: %s %s %s %s", area->area_tag,
				   ...
				   (CHECK_FLAG(rinfo->flag, ISIS_ROUTE_FLAG_ZEBRA_RESYNC)
					    ? "resync"
					    : "not-resync"),
				   (CHECK_FLAG(rinfo->flag, ISIS_ROUTE_FLAG_ACTIVE) ? "active"
										    : "inactive"),
				   buff);
		}
#endif /* EXTREME_DEBUG */
...
static void _isis_route_verify_merge(struct isis_area *area, struct route_table **tables,
				     struct route_table **tables_backup, int tree);
			     ...
			     struct route_table *table_backup, int tree)
{
	...
	for (ALL_LIST_ELEMENTS_RO(area->flex_algos->flex_algos, node, fa)) {
		data = fa->data;
		tables[tables_next] = data->spftree[tree][level - 1]->route_table;
		tables_backup[tables_next] = data->spftree[tree][level - 1]->route_table_backup;
		_isis_route_verify_table(area, tables[tables_next], tables_backup[tables_next],
					 NULL);
		tables_next++;
	}
...
void isis_route_verify_merge(struct isis_area *area, struct route_table *level1_table,
			     ...
			     struct route_table *level2_table_backup, int tree)
{
	...
	struct route_table *tables_backup[] = { level1_table_backup, level2_table_backup, NULL };
	_isis_route_verify_merge(area, tables, tables_backup, tree);
}
	...
	for (int i = 0; tables[i]; i++) {
		...
		for (rnode = route_top(tables[i]); rnode; rnode = srcdest_route_next(rnode)) {
			...
			if (mrinfo) {
				...
				if (CHECK_FLAG(mrinfo->flag, ISIS_ROUTE_FLAG_ACTIVE)) {
					...
					UNSET_FLAG(rinfo->flag, ISIS_ROUTE_FLAG_ZEBRA_SYNCED);
					continue;
				} else if (CHECK_FLAG(rinfo->flag, ISIS_ROUTE_FLAG_ACTIVE)) {
					 ...
					 */
					UNSET_FLAG(mrinfo->flag, ISIS_ROUTE_FLAG_ZEBRA_SYNCED);
				} else if (CHECK_FLAG(mrinfo->flag, ISIS_ROUTE_FLAG_ZEBRA_SYNCED)) {
					continue;
				}
			} else {
...
void isis_route_switchover_nexthop(struct isis_area *area, struct route_table *table, int family,
				   union g_addr *nexthop_addr, ifindex_t ifindex)
{
	...
	if (IS_DEBUG_EVENTS) {
		...
		zlog_debug("%s: initiating fast-reroute %s on VRF %s iface %s", __func__,
			   family2str(family), vrfname ? vrfname : "", ifname ? ifname : "");
	}
	...
	for (rnode = route_top(table); rnode; rnode = srcdest_route_next(rnode)) {
		...
		if (!nexthoplookup(rinfo->nexthops, family, nexthop_addr, ifindex))
			continue;
