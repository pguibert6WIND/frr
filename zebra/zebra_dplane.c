...
dplane_nexthop_update_internal(struct nhg_hash_entry *nhe, enum dplane_op_e op)
{
	...
	if (ret == AOK) {
		if (CHECK_FLAG(nhe->flags, NEXTHOP_GROUP_INITIAL_DELAY_INSTALL)) {
			...
			dplane_ctx_free(&ctx);
			if (op == DPLANE_OP_PIC_NH_INSTALL || op == DPLANE_OP_PIC_NH_UPDATE ||
			    op == DPLANE_OP_PIC_NH_DELETE)
				atomic_fetch_add_explicit(&zdplane_info.dg_pic_nexthops_in, 1,
							  memory_order_relaxed);
			else
	...
	if (op == DPLANE_OP_PIC_NH_INSTALL || op == DPLANE_OP_PIC_NH_UPDATE ||
	    op == DPLANE_OP_PIC_NH_DELETE)
		atomic_fetch_add_explicit(&zdplane_info.dg_pic_nexthops_in, 1, memory_order_relaxed);
	else
		atomic_fetch_add_explicit(&zdplane_info.dg_nexthops_in, 1, memory_order_relaxed);
...
int dplane_show_helper(struct vty *vty, bool detailed)
{
	...
	incoming = atomic_load_explicit(&zdplane_info.dg_pic_nexthops_in, memory_order_relaxed);
	errs = atomic_load_explicit(&zdplane_info.dg_pic_nexthop_errors, memory_order_relaxed);
