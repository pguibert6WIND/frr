			   ...
			   afi_t afi, bool from_dplane)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL) {
		if (*nhe && !CHECK_FLAG((*nhe)->nhg.flags, NEXTHOP_GROUP_TYPE_GROUP))
			...
			zlog_debug("%s: id %u, lookup %p, vrf %d, type %d, depends %p%s => Found %p(group %u)",
				   __func__, lookup->id, lookup, lookup->vrf_id,
				   lookup->type, nhg_depends,
				   (from_dplane ? " (from dplane)" : ""), *nhe, (*nhe)->nhg.group->id_grp);
	}
...
zebra_nhg_rib_find_nhe(struct nhg_hash_entry *rt_nhe, afi_t rt_afi)
{
	...
	if (IS_ZEBRA_DEBUG_NHG_DETAIL) {
		if (CHECK_FLAG(rt_nhe->nhg.flags, NEXTHOP_GROUP_TYPE_GROUP))
			zlog_debug("%s: rt_nhe %p(%pNG) => nhe %p(group %u)", __func__,
				   rt_nhe, rt_nhe, nhe, nhe->nhg.group->id_grp);
		else
					   ...
					   struct nexthop_group *nhg, afi_t afi)
{
	...
	if (!CHECK_FLAG(nhg->flags, NEXTHOP_GROUP_TYPE_GROUP) && !nhg->nexthop) {
		if (IS_ZEBRA_DEBUG_NHG)
