	...
	*nhe = newnhe;
	...
	if (createdPic && !pic) {
		...
		if (pic_nhe && ((*nhe)->pic_nhe) == NULL) {
			...
			SET_FLAG(pic_nhe->flags,
				 NEXTHOP_GROUP_PIC_NON_RECURSIVE);
		}
	}
			...
			struct nhg_hash_entry *nhe, afi_t afi, bool from_dplane)
{
	...
	for (nh = nhe->nhg.nexthop; nh; nh = nh->next) {
		...
		ret = nexthop_group_add_sorted_nodup(&pic_nh_lookup.nhg,
						     pic_nexthop_tmp);
		if (!ret)
	...
	*pic_nhe = picnhe;
	if (pic_nh_lookup.nhg.nexthop)
		nexthops_free(pic_nh_lookup.nhg.nexthop);
	if (IS_ZEBRA_DEBUG_NHG_DETAIL)
		zlog_debug("%s: create PIC nhe id %d for nhe %d", __func__,
			   picnhe->id, nhe->id);
	return created;
}
...
static struct nhg_hash_entry *
depends_find_recursive(const struct nexthop *nh, afi_t afi, int type, bool pic)
{
	struct nhg_hash_entry *nhe;
...
struct nhg_hash_entry *zebra_nhg_rib_find(uint32_t id, struct nexthop_group *nhg,
...
void zebra_nhg_dplane_result(struct zebra_dplane_ctx *ctx)
{
	...
	if (op == DPLANE_OP_NH_DELETE || DPLANE_OP_PIC_CONTEXT_DELETE) {
