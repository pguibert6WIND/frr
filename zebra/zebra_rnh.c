	...
	if (re) {
		...
		if (CHECK_FLAG(nhg->flags, NEXTHOP_GROUP_TYPE_GROUP)) {
			for (nhgid = re->nhe->nhg.group; nhgid;
			     nhgid = nhgid->next) {
				if (nhgid->nhg) {
					num += zebra_send_rnh_update_nhg(s, re,
									 nhg,
									 message);
					if (num < 0)
						goto failure;
			...
			num += zebra_send_rnh_update_nhg(s, re, nhg, message);
			if (num < 0)
				goto failure;
		}
		...
		if (nhg) {
			if (CHECK_FLAG(nhg->flags, NEXTHOP_GROUP_TYPE_GROUP)) {
				...
				num += zebra_send_rnh_update_nhg(s, re, nhg,
								 0 /* message */);
				if (num < 0)
					goto failure;
			}
			      ...
			      const struct route_entry *re)
{
	...
	nexthop_json_helper(json_nexthop, nexthop, display_vrfid);
}
			       ...
			       const struct nexthop *nexthop)
{
	...
	if (re == NULL || nexthop->vrf_id != re->vrf_id)
        ...
        nexthop_vty_helper(vty, nexthop, display_vrfid);
}
