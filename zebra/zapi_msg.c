...
static bool zapi_read_nexthop_group_backup(struct zserv *client,
...
static bool zapi_read_nexthop_group(struct zserv *client,
				    uint16_t nexthopgroup_num, uint32_t *id_grp,
				    struct nexthop_group **png)
{
	struct nexthop_group *nhg = NULL;
	struct nexthop_group nhg_tmp = {};
	uint16_t i;
	uint32_t nhgid;
	struct nhg_hash_entry *nhe;
	 ...
	 * a copy must be done in order to sort properly the list
	 */
	nexthop_group_copy_nh_sorted(nhg, nhg_tmp.nexthop);

	/* Then, the original nhg_tmp.nexthop list is suppressed */
...
static void zread_nhg_child_add(ZAPI_HANDLER_ARGS)
{
	...
	if ((!zapi_read_nexthop_group(client, api_nhg_group.child_group_num,
				      &api_nhg_group.child_group_id[0], &nhg) ||
	     !zapi_read_nexthop_group_backup(client,
					     api_nhg_group.backup_child_group_num,
					     &api_nhg_group
						      .backup_child_group_id[0],
					     &bnhg))) {
		flog_warn(EC_ZEBRA_NEXTHOP_CREATION_FAILED,
			  "%s: Nexthop Group Creation failed", __func__);
