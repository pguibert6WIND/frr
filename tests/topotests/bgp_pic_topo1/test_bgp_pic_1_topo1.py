...
"""
 test_bgp_pic_topo1.py: Test the FRR BGP daemon with bgp PIC functionality
...
def route_check_nhg_id_is_zebra(ipaddr_str, rname, vrf_name=None):
    tgen = get_topogen()
...
def test_bgp_ipv4_route_uses_nexthop_group():
    ...
    nhg_id_1 = route_check_nhg_id_is_zebra("192.0.2.9/32", "r1")
    nhg_id_2 = route_check_nhg_id_is_zebra("192.0.2.9/32", "r1", vrf_name="vrf1")
...
def test_bgp_ipv4_convergence_igp():
    ...
    """
    global pic_nhg_2, installed_nhg_2

    tgen = get_topogen()
    if tgen.routers_have_failure():
        pytest.skip(tgen.errors)

    step(f"Check that 192.0.2.9/32 unicast entry has 2 paths now")
    expected = {
        "192.0.2.9/32": [
            {
                "prefix": "192.0.2.9/32",
    ...
    step(f"Check that 192.0.2.9/32 unicast used PIC context")
    output = json.loads(tgen.gears["r1"].vtysh_cmd("show ip route 192.0.2.9/32 json"))
    pic_nhg_1 = output["192.0.2.9/32"][0]["installedPicNexthopGroupId"]
    installed_nhg_1 = output["192.0.2.9/32"][0]["installedNexthopGroupId"]
    logger.info(f"nhg IDs for 192.0.2.9/32 on vrf main: PIC {pic_nhg_1}, installed {installed_nhg_1}")

    assert pic_nhg_1 != 0, "PIC NHG ID should be different from 0"

    step("Check that 192.0.2.9/32 has 2 paths in Linux")
    expected = [
        {
            "dst": "192.0.2.9",
            "nhid": installed_nhg_1,
            "protocol": "bgp",
            "metric": 20,
            "nexthops": [
                {
    ...
    step(f"Check that 192.0.2.9/32 mpls vpn entry has 2 paths now")
    expected = {
        "192.0.2.9/32": [
            {
                "prefix": "192.0.2.9/32",
    ...
    step("Check that 192.0.2.9/32 mpls vpn entry used PIC context")
    output = json.loads(tgen.gears["r1"].vtysh_cmd("show ip route vrf vrf1 192.0.2.9/32 json"))
    pic_nhg_2 = output["192.0.2.9/32"][0]["installedPicNexthopGroupId"]
    installed_nhg_2 = output["192.0.2.9/32"][0]["installedNexthopGroupId"]
    logger.info(f"nhg IDs for 192.0.2.9/32 mpls vpn entry on vrf vrf1: PIC {pic_nhg_2}, installed {installed_nhg_2}")

    assert pic_nhg_2 != 0, "PIC NHG ID should be different from 0"
    step("Check that 192.0.2.9/32 mpls vpn entry and 192.0.2.9/32 of vrf default use same PIC context")
    assert pic_nhg_2 == pic_nhg_1, "PIC NHG ID from mpls vpn entry should be the same as PIC NHG ID from vrf default"

    step("Check that 192.0.2.9/32 mpls vpn entry has 2 paths now in Linux")
    expected = [
        {
            "dst": "192.0.2.9",
            "nhid": installed_nhg_2,
            "protocol": "bgp",
            "metric": 20,
            "nexthops": [
                {
...
def test_bgp_ipv4_convergence_igp_label_changed():
    ...
    step("Check that 192.0.2.9/32 mpls vpn entry has not moved since before the IGP label change")
    output = json.loads(tgen.gears["r1"].vtysh_cmd("show ip route vrf vrf1 192.0.2.9/32 json"))
    pic_nhg_mpls = output["192.0.2.9/32"][0]["installedPicNexthopGroupId"]
    installed_nhg_mpls = output["192.0.2.9/32"][0]["installedNexthopGroupId"]
    logger.info(f"nhg IDs for 192.0.2.9/32 mpls vpn entry on vrf vrf1: PIC {pic_nhg_mpls}, installed {installed_nhg_mpls}")
    assert pic_nhg_2 == pic_nhg_mpls, "PIC NHG ID for 192.0.2.9/32 mpls vpn entry should be the same as before the IGP label change"

    step("Check that 192.0.2.9/32 mpls vpn entry has its installed NHG ID that changed from before the IGP label change")
    assert installed_nhg_mpls != pic_nhg_mpls, "installed NHG ID for 192.0.2.9/32 mpls vpn entry should be different from before the IGP label change"

    step("Check that 192.0.2.9/32 mpls vpn entry uses the IGP label 16055 in Linux")
    expected = [
        {
            "dst": "192.0.2.9",
            "nhid": installed_nhg_mpls,
            "protocol": "bgp",
            "metric": 20,
            "nexthops": [
                {
    ...
    installed_nhg_2 = installed_nhg_mpls
    pic_nhg_2 = pic_nhg_mpls
