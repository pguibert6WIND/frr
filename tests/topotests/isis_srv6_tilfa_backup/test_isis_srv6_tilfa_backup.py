...
def test_kernel_srv6_routes_step1():
    ...
    for rname in ["rt1", "rt2", "rt3", "rt4"]:
        ...
        # Verify local SID is installed (the router's own locator)
        # Each router should have its own locator as a local SID
        locator_map = {
            "rt1": "fc00:0:1::/48",
            "rt2": "fc00:0:2::/48",
            "rt3": "fc00:0:3::/48",
        ...
        # The local SID should be installed with seg6local action
        ...
        for other_rname, other_locator in locator_map.items():
            if other_rname != rname:
                assert other_locator in routes, \
                    f"{rname}: route to {other_rname} ({other_locator}) not in kernel"
