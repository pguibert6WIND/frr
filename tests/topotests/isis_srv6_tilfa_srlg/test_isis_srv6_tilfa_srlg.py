...
Test IS-IS TI-LFA with SRv6 and SRLG (Shared Risk Link Group) protection.

This test extends the basic SRv6 TI-LFA test to verify SRLG protection
functionality. SRLG protection ensures that backup paths avoid links
that share the same risk group as the protected link.

SRLG Configuration:
- RT2's eth-rt4-1 and eth-rt4-2 are both assigned SRLG 100
- RT4's eth-rt2-1 and eth-rt2-2 are both assigned SRLG 100
- With SRLG protection enabled, backup paths from RT2 to RT4 should NOT
  use the alternate direct link (same SRLG) but instead route through
  RT3-RT5 path.
...
# Step 4
#
# Test SRLG (Shared Risk Link Group) protection
#
