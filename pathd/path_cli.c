...
static int segment_list_has_srv6_src_dst(struct vty *vty, char *xpath, long index,
					 const char *index_str, struct in6_addr adj_src_ipv6,
					 ...
					 const char *adj_src_ipv6_str, const char *adj_dst_ipv6_str)
{
	...
	/* type */
	snprintf(xpath, XPATH_MAXLEN, "./segment[index='%s']/nai/type", index_str);
	if (adj_src_ipv6_str != NULL) {
		nb_cli_enqueue_change(vty, xpath, NB_OP_MODIFY, "ipv6_srv6_adjacency");
		node_src_id = adj_src_ipv6_str;
	} else {
		/*
		 * This is just to make the compiler happy about
	...
	/* addresses */
	snprintf(xpath, XPATH_MAXLEN, "./segment[index='%s']/nai/local-address", index_str);
	nb_cli_enqueue_change(vty, xpath, NB_OP_MODIFY, node_src_id);
	snprintf(xpath, XPATH_MAXLEN, "./segment[index='%s']/nai/remote-address", index_str);
...
/* clang-format on */
{
	...
	if (has_adj != NULL) {
		...
		status = segment_list_has_srv6_src_dst(vty, xpath, index, index_str,
						       adj_srv6_src_ipv6, adj_srv6_dst_ipv6,
					...
					bool show_defaults)
{
	...
	if (yang_dnode_exists(dnode, "nai")) {
		...
		case SRTE_SEGMENT_NAI_TYPE_IPV6_SRV6_ADJACENCY:
			yang_dnode_get_ip(&addr, dnode, "nai/local-address");
			yang_dnode_get_ip(&addr_rmt, dnode, "./nai/remote-address");
			vty_out(vty, " nai adjacency-srv6 %pI6", &addr.ipaddr_v6);
			vty_out(vty, " %pI6", &addr_rmt.ipaddr_v6);
			break;
		default:
			break;
	...
	"Use segment routing ipv6 sid manager\n")
{
	...
	snprintf(xpath, sizeof(xpath), "/frr-pathd:pathd/srte/use-srv6-sid-manager");
