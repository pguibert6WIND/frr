					 ...
					 struct in6_addr *ted_sid_srv6)
{
	...
	if (ted_sid != 0) {
		if (s_list->protocol_origin == SRTE_ORIGIN_LOCAL && s_entry->sid_value != ted_sid) {
			s_entry->sid_value = ted_sid;
			SET_FLAG(s_list->flags, F_SEGMENT_LIST_MODIFIED);
		}
	}
	if (ted_sid_srv6 != NULL) {
		if (s_list->protocol_origin == SRTE_ORIGIN_LOCAL &&
		    !IPV6_ADDR_SAME(&s_entry->srv6_sid_value, ted_sid_srv6)) {
			IPV6_ADDR_COPY(&s_entry->sid_value, ted_sid_srv6);
			SET_FLAG(s_list->flags, F_SEGMENT_LIST_MODIFIED);
		}
	}
}
					 ...
					 struct in6_addr *srv6_binding_sid)
{
	...
	if (!srv6_binding_sid ||
	    (srv6_binding_sid && !IPV6_ADDR_SAME(&policy->srv6_binding_sid, srv6_binding_sid))) {
	...
	if (srv6_binding_sid) {
		...
		IPV6_ADDR_COPY(&policy->srv6_binding_sid, &srv6_binding_sid_zero);

	/* Reinstall the Binding-SID if necessary. */
	if (policy->best_candidate) {
		path_zebra_add_sr_policy(policy, policy->best_candidate->lsp->segment_list);
				 ...
				 struct prefix *prefix_cli, uint32_t algo)
{
	...
	if (CHECK_SID(entry->segment_list->protocol_origin, ted_sid,
		...
		srte_segment_set_local_modification(entry->segment_list, entry, ted_sid, NULL);
	return status;
}
				 ...
				 struct ipaddr *remote)
{
	...
	if (path_ted_query_type_k(local, remote, &ted_sid) < 0) {
		zlog_warn("%s:SL:  ERROR query K : ted-sid", __func__);
	} else {
