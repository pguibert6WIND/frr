...
static const char PCEP_VTYSH_ARG_MSD_SRV6_ENDD[] = "end-d";
static const char PCEP_VTYSH_ARG_MSD_SRV6_ENDPOP[] = "end-pop";
static const char PCEP_VTYSH_ARG_MSD_SRV6_HENCAPS[] = "h-encaps";
static const char PCEP_VTYSH_ARG_MSD_SRV6_SEGSLEFT[] = "segs-left";
...
static const char PCEP_CLI_CAP_PCC_RESOLVE_SRV6_NAI[] = " [PCC can resolve NAI to SRv6 SID]";
...
short pcc_msd_srv6_end_d = SRV6_MSD_DEFAULT_END_D;
short pcc_msd_srv6_h_encaps = SRV6_MSD_DEFAULT_H_ENCAPS;
short pcc_msd_srv6_end_pop = SRV6_MSD_DEFAULT_END_POP;
short pcc_msd_srv6_segs_left = SRV6_MSD_DEFAULT_SEGS_LEFT;
...
static int path_pcep_cli_pcc_pcc_msd_srv6_reset_max(struct vty *vty, const char *maxendd,
						    const char *maxendpop, const char *maxhencaps,
						    const char *maxsegsleft)
{
	if (maxendd)
		pcc_msd_srv6_end_d_configured = false;
...
static int path_pcep_cli_pcc_pcc_msd_srv6_max(struct vty *vty, long maxendd, long maxendpop,
					      long maxhencaps, long maxsegsleft)
{
	if (maxendd) {
		pcc_msd_srv6_end_d_configured = true;
		PCEP_VTYSH_INT_ARG_CHECK(PCEP_VTYSH_ARG_MSD_SRV6_ENDD, maxendd, pcc_msd_srv6_end_d,
					 0, 33);
	}
	if (maxendpop) {
		pcc_msd_srv6_end_pop_configured = true;
		PCEP_VTYSH_INT_ARG_CHECK(PCEP_VTYSH_ARG_MSD_SRV6_ENDPOP, maxendpop,
					 pcc_msd_srv6_end_pop, 0, 33);
	}
	if (maxhencaps) {
		pcc_msd_srv6_h_encaps_configured = true;
		PCEP_VTYSH_INT_ARG_CHECK(PCEP_VTYSH_ARG_MSD_SRV6_HENCAPS, maxhencaps,
					 pcc_msd_srv6_h_encaps, 0, 33);
	}

	if (maxsegsleft) {
		pcc_msd_srv6_segs_left_configured = true;
		PCEP_VTYSH_INT_ARG_CHECK(PCEP_VTYSH_ARG_MSD_SRV6_SEGSLEFT, maxsegsleft,
					 pcc_msd_srv6_segs_left, 0, 33);
	}
	return CMD_SUCCESS;
}
...
int pcep_cli_pcc_config_write(struct vty *vty)
{
	...
	if (pcc_msd_srv6_segs_left_configured) {
		vty_out(vty, "    %s %d\n", PCEP_VTYSH_ARG_MSD_SRV6_SEGSLEFT,
			pcc_msd_srv6_segs_left);
		lines++;
	}
...
DEFPY(pcep_cli_pcc_pcc_msd_srv6,
      ...
      "msd-srv6 [end-d (1-255)$maxendd] [end-pop (1-255)$maxendpop] [h-encaps (1-255)$maxhencaps] [segs-left (1-255)$maxsegsleft]",
      "Maximum number of SIDs for SRv6\n"
      "Maximum number of SIDs present in an SRH when performing decapsulation for SRv6\n"
      "Maximum number of SIDs value (0-255)\n"
...
DEFPY(no_pcep_cli_pcc_pcc_msd_srv6,
      ...
      "no msd-srv6 [end-d$maxendd_str] [end-pop$maxendpop_str] [h-encaps$maxhencaps_str] [segs-left$maxsegsleft_str]",
      NO_STR
      "Maximum number of SIDs for SRv6\n"
      "Maximum number of SIDs present in an SRH when performing decapsulation for SRv6\n"
      "Maximum number of SIDs in the SRH to which the router can apply PSP or USP behaviors\n"
      ...
      "Maximum number of SIDs in the SRH before applying the End behavior\n")
{
	return path_pcep_cli_pcc_pcc_msd_srv6_reset_max(vty, maxendd_str, maxendpop_str,
							maxhencaps_str, maxsegsleft_str);
}
