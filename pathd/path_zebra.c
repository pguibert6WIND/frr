			  ...
			  enum seg6local_action_t action, struct in6_addr *srv6_segs, int num_segs)
{
	...
	if (num_segs == 0)
		return (void)zclient_route_send(ZEBRA_ROUTE_DELETE, pathd_zclient, &api);
					      ...
					      struct path_nht_data *nhtd)
{
	...
	if (znh && !sid_zero_ipv6(&policy->srv6_binding_sid) && segment_list &&
		...
		(void)path_zebra_send_bsid(&policy->srv6_binding_sid, znh->ifindex,
...
static int path_zebra_process_srv6_locator_add(ZAPI_CALLBACK_ARGS)
{
	...
	if (zapi_srv6_locator_decode(pathd_zclient->ibuf, &locator) < 0)
		return -1;

	if (!path_zebra_locator_ctx_lookup_by_name((const char *)&locator.name)) {
		ctx = XCALLOC(MTYPE_LOCATOR_LIST_CTX, sizeof(struct path_locator_ctx));
	...
	RB_FOREACH (policy, srte_policy_head, &srte_policies) {
		...
		memcpy(&sid_ctx.nh6, &policy->endpoint.ip._v6_addr, sizeof(struct in6_addr));
		sid_ctx.color = policy->color;
		if (!CHECK_FLAG(policy->flags, F_POLICY_BSID_ALLOCATED))
			path_zebra_srv6_manager_get_sid(&sid_ctx, &policy->srv6_binding_sid);
	}
	return 0;
}
...
static int path_zebra_process_srv6_locator_delete(ZAPI_CALLBACK_ARGS)
{
	...
	if (zapi_srv6_locator_decode(pathd_zclient->ibuf, &loc) < 0)
		return -1;
	...
	RB_FOREACH (policy, srte_policy_head, &srte_policies) {
		...
		if (policy->srv6_locator == locator) {
			...
			(void)path_zebra_send_bsid(&policy->srv6_binding_sid, 0,
						   ZEBRA_SEG6_LOCAL_ACTION_END_B6_ENCAP, NULL, 0);
			if (CHECK_FLAG(policy->flags, F_POLICY_BSID_ALLOCATED))
                          path_zebra_srv6_manager_release_sid(&ctx, locator->name);
...
static int path_zebra_srv6_sid_notify(ZAPI_CALLBACK_ARGS)
{
	...
	if (!zapi_srv6_sid_notify_decode(pathd_zclient->ibuf, &ctx, &sid_addr, &sid_func, NULL, &note,
					 loc_name, sizeof(loc_name))) {
	...
	PATH_ZEBRA_DEBUG("%s: received SRv6 SID notify: ctx %s sid_value %pI6 %s", __func__,
	...
	RB_FOREACH_SAFE (policy, srte_policy_head, &srte_policies, safe_pol) {
		...
		case ZAPI_SRV6_SID_ALLOCATED:
			PATH_ZEBRA_DEBUG("SRv6 SID %pI6 %s : ALLOCATED", &sid_addr,
					 srv6_sid_ctx2str(buf, sizeof(buf), &ctx));
			SET_FLAG(policy->flags, F_POLICY_BSID_ALLOCATED);

			context = path_zebra_locator_ctx_lookup_by_name((const char *)locator_name);
			if (context)
				policy->srv6_locator = context->locator;
			if (policy->best_candidate)
				path_zebra_add_sr_policy(policy,
							 policy->best_candidate->lsp->segment_list);
			break;
		...
		case ZAPI_SRV6_SID_FAIL_ALLOC:
			PATH_ZEBRA_DEBUG("SRv6 SID %pI6 %s: Failed to allocate", &sid_addr,
					 srv6_sid_ctx2str(buf, sizeof(buf), &ctx));
...
struct path_locator_ctx *path_zebra_locator_ctx_lookup_by_name(const char *locator_name)
{
	...
	frr_each_safe (path_locator_list, &path_srv6_locator_list, ctx) {
		if (ctx->locator && strcmp(ctx->locator->name, locator_name) == 0) {
			return ctx;
		}
	}
	return NULL;
...
void path_zebra_srv6_manager_get_sid(struct srv6_sid_ctx *ctx, struct in6_addr *sid_addr)
{
	...
	ret = srv6_manager_get_sid(pathd_zclient, ctx, sid_addr, NULL, &sid_func, false);
	if (ret < 0)
...
void path_zebra_srv6_manager_release_sid(struct srv6_sid_ctx *ctx, const char *locator_name)
{
	...
	ret = srv6_manager_release_sid(pathd_zclient, ctx, locator_name, false);
	if (ret < 0)
		/* TODO: need to re-send later GET_SID message */
