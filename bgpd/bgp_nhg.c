...
static __attribute__((__unused__)) unsigned int
bgp_nhg_childs_count(const struct bgp_nhg_cache *nhg)
{
	return bgp_nhg_connected_tree_count(&nhg->nhg_childs);
}

static __attribute__((__unused__)) bool
bgp_nhg_childs_is_empty(const struct bgp_nhg_cache *nhg)
{
	return bgp_nhg_connected_tree_is_empty(&nhg->nhg_childs);
}

static __attribute__((__unused__)) void
bgp_nhg_childs_del(struct bgp_nhg_cache *from, struct bgp_nhg_cache *depend)
{
...
static __attribute__((__unused__)) unsigned int
bgp_nhg_parents_count(const struct bgp_nhg_cache *nhg)
{
	return bgp_nhg_connected_tree_count(&nhg->nhg_parents);
}

static __attribute__((__unused__)) void
bgp_nhg_parents_del(struct bgp_nhg_cache *from, struct bgp_nhg_cache *dependent)
{
...
void bgp_nhg_debug_parent(uint32_t child_ids[], int count, char *group_buf,
			  size_t len)
{
	int i;
	char *ptr = group_buf;
...
/* display in a debug trace BGP NHG information, with a custom 'message' string */
static void bgp_nhg_debug(struct bgp_nhg_cache *nhg, const char *message)
{
	char nexthop_buf[BGP_NEXTHOP_BUFFER_SIZE];

	if (CHECK_FLAG(nhg->flags, BGP_NHG_FLAG_TYPE_PARENT)) {
		bgp_nhg_debug_parent(nhg->childs.childs, nhg->childs.child_num,
				     nexthop_buf, sizeof(nexthop_buf));
		zlog_debug("NHG %u: child %s (%s)", nhg->id, message,
			   nexthop_buf);
		return;
	}
	...
	if (nhg->nexthops.nexthop_num > 1) {
		zlog_debug("NHG %u: %s", nhg->id, message);
	...
	bgp_debug_zebra_nh_buffer(&nhg->nexthops.nexthops[0], nexthop_buf,
				  sizeof(nexthop_buf));
	zlog_debug("NHG %u: %s (%s)", nhg->id, message, nexthop_buf);
}
...
struct bgp_nhg_cache *
bgp_nhg_parent_find_per_child(struct bgp_path_info *p_mpinfo[],
			      ...
			      struct bgp_nhg_cache *lookup)
{
	...
	/* look in current pi->bgp_nhg_parent in priority */
	for (i = 0; i < *valid_nh_count; i++) {
		if (p_mpinfo[i] && p_mpinfo[i]->bgp_nhg_parent &&
		    bgp_nhg_parent_cache_compare_childs(lookup,
							p_mpinfo[i]
								->bgp_nhg_parent))
			return p_mpinfo[i]->bgp_nhg_parent;
	}
					    ...
					    struct zapi_nhg_group *api_nhg_group)
{
	...
	for (i = 0; i < bgp_nhg->childs.child_num; i++) {
		...
		if (!child_nhg ||
		    !CHECK_FLAG(child_nhg->state, BGP_NHG_STATE_INSTALLED)) {
			if (BGP_DEBUG(nexthop_group, NEXTHOP_GROUP_DETAIL))
				zlog_warn("%s: nhg %u not sent, child NHG ID %u not present or not installed.",
					  __func__, bgp_nhg->id,
					  bgp_nhg->childs.childs[i]);
			continue;
		}
 ...
 * The parent entry is kept with the new list of child nexthop-groups.
 */
					     ...
					     struct bgp_nhg_cache *nhg_child)
{
	...
	if (BGP_DEBUG(nexthop_group, NEXTHOP_GROUP)) {
		bgp_debug_zebra_nh_buffer(&nhg_child->nexthops.nexthops[0],
					  nexthop_buf, sizeof(nexthop_buf));
		zlog_debug("NHG %u: detaching ID %u child (%s)", nhg->id,
	...
	LIST_FOREACH_SAFE (path, &(nhg->paths), nhg_parent_cache_thread, safe) {
		if (!path->bgp_nhg) {
			LIST_REMOVE(path, nhg_parent_cache_thread);
			path->bgp_nhg_parent = NULL;
			nhg->path_count--;
		}
	}
	...
	if (BGP_DEBUG(nexthop_group, NEXTHOP_GROUP)) {
		zlog_debug("NHG %u: removing child id %u (%u -> %u)", nhg->id,
			   nhg_child->id, nhg->childs.child_num + 1,
			   nhg->childs.child_num);
	}
...
static void bgp_nhg_connected_del(struct bgp_nhg_cache *nhg)
{
	...
	frr_each_safe (bgp_nhg_connected_tree, &(nhg->nhg_parents),
		       rb_node_dep) {
...
void bgp_nhg_path_child_unlink(struct bgp_path_info *pi, bool force)
{
	struct bgp_nhg_cache *nhg;
...
static void bgp_nhg_path_unlink_internal(struct bgp_path_info *pi, bool free_nhg)
{
	...
	nhg = pi->bgp_nhg_parent;
	bgp_nhg_path_child_unlink(pi, true);

	if (!nhg)
		return;

	LIST_REMOVE(pi, nhg_parent_cache_thread);
	nhg->path_count--;
	pi->bgp_nhg_parent = NULL;
...
void bgp_nhg_id_set_installed(uint32_t id)
{
	...
	if (BGP_DEBUG(nexthop_group, NEXTHOP_GROUP))
		zlog_debug("NHG %u: ID is installed, update dependent routes",
			   nhg->id);
	LIST_FOREACH (path, &(nhg->paths), nhg_parent_cache_thread) {
...
static void bgp_nhg_remove_nexthops(struct bgp_nhg_cache *nhg)
{
	...
	frr_each_safe (bgp_nhg_connected_tree, &nhg->nhg_parents, rb_node_dep) {
		...
		LIST_FOREACH_SAFE (path, &(parent_nhg->paths),
				   nhg_parent_cache_thread, safe) {
			if (path->bgp_nhg == nhg) {
				LIST_REMOVE(path, nhg_parent_cache_thread);
				path->bgp_nhg_parent = NULL;
	...
	struct bgp_nhg_cache *child)
{
	...
	LIST_FOREACH_SAFE (path, &(nhg->paths), nhg_parent_cache_thread, safe) {
		...
		if (is_default_path) {
			...
			continue;
		/* child nexthop and parent nexthop must not be removed
		 * in this function call, because they are used to parse
		 * the paths registered. Hence the below 2 calls, where
		 * the none of the nexthop-groups are removed.
		 */
		bgp_nhg_path_child_unlink(path, false);
...
void bgp_nhg_refresh_by_nexthop(struct bgp_nexthop_cache *bnc)
{
	...
	frr_each_safe (bgp_nhg_cache, &nhg_cache_table, nhg) {
		...
		if (found) {
			...
			if (bgp_nhg_detach_paths_resolved_over_prefix(
				   ...
				   json_object *json, bool detail)
{
	...
	if (CHECK_FLAG(nhg->flags, BGP_NHG_FLAG_TYPE_PARENT)) {
		if (bgp_nhg_childs_count(nhg)) {
			...
			if (json_array)
				json_object_object_add(json, "childList",
						       json_array);
			else
				vty_out(vty, "\n");
		}
	...
	if (bgp_nhg_parents_count(nhg)) {
		if (json) {
			json_object_int_add(json, "parentListCount",
...
{
	qsort(grp, nhg_num, sizeof(uint32_t), &uint32_cmp);
}
