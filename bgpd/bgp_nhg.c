...
static void bgp_nhg_path_unlink_and_free(struct bgp_path_info *pi, bool free_nhg)
{
	struct bgp_nhg_cache *nhg;
...
{
	return bgp_nhg_path_unlink_and_free(pi, true);
}
					      ...
					      struct prefix *resolved_prefix)
{
	...
	LIST_FOREACH_SAFE (path, &(nhg->paths), nhg_cache_thread, safe) {
		 ...
		 */
		bgp_nhg_path_unlink_and_free(path, false);
