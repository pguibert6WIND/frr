 ...
 * This second hash list is necessary when using frr_each_safe() when modifying
 * nexthops and groups in nhg cache table: each NHG nexthop may be detached, and
 * the dependent NHG groups to; frr_each_safe() stores a backup of the next
 * pointer of the nexthop, but it does not have a pointer to the detached NHG
 * group. Using a secondary hash list helps handling this issue.
 */
...
/* BGP NHGs are either nexthops or groups. To reflect dependency between a group
 * and its nexthops (and reversely reflect that a nexthop is used by a list of
 * groups), a tree of 'bgp_nhg_connected' structures are used. The below API is
 * used to handle those structures.
 */
...
static unsigned int bgp_nhg_child_groups_count(const struct bgp_nhg_cache *nhg)
{
	return bgp_nhg_connected_tree_count(&nhg->nhg_child_groups);
}

static bool bgp_nhg_child_groups_is_empty(const struct bgp_nhg_cache *nhg)
{
	return bgp_nhg_connected_tree_is_empty(&nhg->nhg_child_groups);
}

static void bgp_nhg_child_groups_del(struct bgp_nhg_cache *from,
				     struct bgp_nhg_cache *depend)
{
	bgp_nhg_connected_tree_del_nhg(&from->nhg_child_groups, depend);
}

static void bgp_nhg_child_groups_init(struct bgp_nhg_cache *nhg)
{
	bgp_nhg_connected_tree_init(&nhg->nhg_child_groups);
}

static unsigned int bgp_nhg_parent_groups_count(const struct bgp_nhg_cache *nhg)
{
	return bgp_nhg_connected_tree_count(&nhg->nhg_parent_groups);
}

static void bgp_nhg_parent_groups_del(struct bgp_nhg_cache *from,
				      struct bgp_nhg_cache *dependent)
{
	bgp_nhg_connected_tree_del_nhg(&from->nhg_parent_groups, dependent);
}

static void bgp_nhg_parent_groups_init(struct bgp_nhg_cache *nhg)
{
	bgp_nhg_connected_tree_init(&nhg->nhg_parent_groups);
}
					    ...
					    struct zapi_nhg_group *api_nhg_group)
{
	...
	for (i = 0; i < bgp_nhg->groups.group_num; i++) {
		...
		api_nhg_group->child_group_id[i] = bgp_nhg->groups.groups[i];
...
static void bgp_nhg_add_or_update_nhg(struct bgp_nhg_cache *bgp_nhg)
{
	...
	if (CHECK_FLAG(bgp_nhg->flags, BGP_NHG_FLAG_TYPE_GROUP)) {
		...
		if (bgp_nhg_add_or_update_nhg_group(bgp_nhg, &api_nhg_group))
			zclient_nhg_child_send(zclient, ZEBRA_NHG_CHILD_ADD,
					       &api_nhg_group);
		return;
	}
				  ...
				  uint32_t api_group[])
{
	...
	LIST_INIT(&(nhg->paths));
	bgp_nhg_parent_groups_init(nhg);
	bgp_nhg_child_groups_init(nhg);
...
/* Used to sync the BGP NHG ID hash table with the BGP NHG hash table. After a
 * nexthop is removed, some BGP NHG group may be empty. This function detects it
 * and frees it
 */
static void bgp_nhg_unused_clean(void)
{
	struct bgp_nhg_cache *nhg;
					 ...
					 struct bgp_nhg_cache *nhg_nexthop)
{
	...
	bgp_nhg_parent_groups_del(nhg_nexthop, nhg);
	bgp_nhg_child_groups_del(nhg, nhg_nexthop);
...
void bgp_nhg_del_nhg(struct bgp_nhg_cache *nhg)
{
	...
	frr_each_safe (bgp_nhg_connected_tree, &(nhg->nhg_parent_groups),
		       rb_node_dep) {
	...
	frr_each_safe (bgp_nhg_connected_tree, &(nhg->nhg_child_groups),
		       rb_node_dep) {
		bgp_nhg_parent_groups_del(rb_node_dep->nhg, nhg);
		bgp_nhg_child_groups_del(nhg, rb_node_dep->nhg);
	}
	bgp_nhg_free(nhg);
}
...
static void bgp_nhg_path_unlink_internal(struct bgp_path_info *pi, bool force)
{
	struct bgp_nhg_cache *nhg;
			...
			struct bgp_nhg_cache *nhg_parent)
{
	...
	for (i = 0; i < nexthop_num; i++) {
		bgp_nhg_connected_tree_add_nhg(&nhg_parent->nhg_child_groups,
					       nhg[i]);
		bgp_nhg_connected_tree_add_nhg(&nhg[i]->nhg_parent_groups,
					       nhg_parent);
	}
}
...
/* Called whenever a BGP NHG is resolved over ourselves:
 * Without BGP NHG, in zebra, nexthop_active() compares prefix to install with
 * the resolved route. With BGP NHG, the prefix is unknown, and zebra can not
 * detect if there is a looped resolution. To avoid this corner case, this code
 * block is called to detach a given prefix from its BGP NHG, in order to call
 * route_add/delete without NHG_ID identifier.
 */
...
/* Called whenever a BGP NHG is resolved over default route:
 * Without BGP NHG, in zebra, nexthop_active() detects resolution over default
 * route by comparing the matching routes. With BGP NHG, the prefix is unknown,
 * and zebra can not detect if there is a looped resolution.
 * To avoid this corner case, this code block is called to detach a given prefix
 * from its BGP NHG, in order to call route_add/delete without NHG_ID identifier.
 */
static bool
bgp_nhg_detach_paths_resolved_over_default_route(struct bgp_nhg_cache *nhg)
{
	struct bgp_nhg_connected *rb_node_dep = NULL;
	struct bgp_nhg_cache *dep_nhg;
	struct bgp_path_info *path, *safe;
	...
	frr_each_safe (bgp_nhg_cache_id, &nhg_cache_id_table, nhg) {
		...
		if (found) {
			...
			if (is_default_prefix(&bnc->resolved_prefix) &&
			    bgp_nhg_detach_paths_resolved_over_default_route(nhg))
				continue;
				   ...
				   json_object *json, bool detail)
{
	...
	if (CHECK_FLAG(nhg->flags, BGP_NHG_FLAG_TYPE_GROUP)) {
		if (bgp_nhg_child_groups_count(nhg)) {
			if (json) {
				json_object_int_add(json, "childGroupCount",
						    bgp_nhg_child_groups_count(
							    nhg));
				json_array = json_object_new_array();
			} else {
				vty_out(vty, "          child group count %u\n",
					bgp_nhg_child_groups_count(nhg));
				vty_out(vty, "          child group");
			}
			frr_each_safe (bgp_nhg_connected_tree,
				       &nhg->nhg_child_groups, rb_node_dep) {
				if (json) {
			...
			if (json_array)
				json_object_object_add(json, "childGroupList",
						       json_array);
			else
				vty_out(vty, "\n");
		}
	...
	if (bgp_nhg_parent_groups_count(nhg)) {
		if (json)
			json_array = json_object_new_array();
		else
			vty_out(vty, "          parent group");
		frr_each_safe (bgp_nhg_connected_tree, &nhg->nhg_parent_groups,
			       rb_node_dep) {
			if (json) {
				json_entry = json_object_new_object();
		...
		if (json_array)
			json_object_object_add(json, "parentGroupList",
					       json_array);
		else
			vty_out(vty, "\n");
	}
...
void bgp_nhg_clear_nhg_nexthop(void)
{
	...
	frr_each_safe (bgp_nhg_cache, &nhg_cache_table, nhg) {
		if (!CHECK_FLAG(nhg->flags, BGP_NHG_FLAG_TYPE_GROUP))
			continue;
		if (bgp_nhg_child_groups_is_empty(nhg))
			continue;
		frr_each_safe (bgp_nhg_connected_tree, &nhg->nhg_child_groups,
			       rb_node_dep) {
			dep_nhg = rb_node_dep->nhg;
