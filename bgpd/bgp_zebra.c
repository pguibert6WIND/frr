...
static int bgp_nhg_notify_owner(ZAPI_CALLBACK_ARGS)
{
	...
	case ZAPI_NHG_INSTALLED:
		bgp_nhg_id_set_installed(id, true);
		if (BGP_DEBUG(zebra, ZEBRA))
...
void bgp_zebra_announce_parse_nexthop(struct bgp_path_info *info,
				      const struct prefix *p, struct bgp *bgp,
				      struct zapi_route *api,
				      unsigned int *valid_nh_count, afi_t afi,
				      safi_t safi, uint32_t *nhg_id,
				      ...
				      bool *allow_recursion)
{
	...
	struct bgp_nhg_cache *p_nhg[MULTIPATH_NUM], *p_nhg_parent;
	unsigned int i = 0;
	...
	if (!bgp_option_check(BGP_OPT_NHG) ||
	    info->sub_type == BGP_ROUTE_AGGREGATE || do_wt_ecmp || is_evpn) {
	...
	for (i = 0; i < *valid_nh_count; i++) {
		...
		if (p_mpinfo[i]->bgp_nhg_nexthop != p_nhg[i]) {
			bgp_debug_zebra_nh_buffer(&p_nhg[i]->nexthops.nexthops[0],
						  nexthop_buf,
						  sizeof(nexthop_buf));
			if (p_mpinfo[i]->bgp_nhg_nexthop) {
			...
			p_mpinfo[i]->bgp_nhg_nexthop = p_nhg[i];
			LIST_INSERT_HEAD(&(p_nhg[i]->paths), p_mpinfo[i],
					 nhg_nexthop_cache_thread);
			p_nhg[i]->path_count++;
		}
		p_nhg[i]->last_update = monotime(NULL);
	...
	bgp_nhg_group_sort(nhg_parent.groups.groups,
			   nhg_parent.groups.group_num);
	...
	if (BGP_DEBUG(nexthop_group, NEXTHOP_GROUP_DETAIL))
		bgp_nhg_debug_group(p_nhg_parent->groups.groups,
				    nhg_parent.groups.group_num, nexthop_buf,
				    sizeof(nexthop_buf));
			       ...
			       size_t len)
{
	...
	snprintfrr(nexthop_buf, len, "%s if %u VRF %u wt %" PRIu64 "%s %s %s",
		   nh_buf, api_nh->ifindex, api_nh->vrf_id, api_nh->weight,
			  ...
			  struct bgp *bgp)
{
	...
	if (bgp_debug_zebra(p)) {
		zlog_debug(
			"Tx route %s %s %pFX metric %u tag %" ROUTE_TAG_PRI
			" count %d nhg %d",
			is_add ? "add" : "delete", bgp->name_pretty, &api.prefix,
