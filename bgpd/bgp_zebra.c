...
static void bgp_zebra_nexthop_group_unlink(
	struct bgp_path_info *p_mpinfo[], unsigned int *valid_nh_count)
{
	unsigned int i;
...
static void bgp_zebra_nexthop_group_configure(
	struct bgp_path_info *info, const struct prefix *p, struct bgp *bgp,
	struct zapi_route *api, unsigned int *valid_nh_count, uint32_t *nhg_id,
	bool *allow_recursion, struct bgp_path_info *p_mpinfo[])

{
	...
	for (i = 0; i < *valid_nh_count; i++) {
		...
		if (api->nexthops[i].type == NEXTHOP_TYPE_IFINDEX ||
			...
			bgp_zebra_nexthop_group_unlink(p_mpinfo, valid_nh_count);
			return;
		}
	...
	for (i = 0; i < *valid_nh_count; i++) {
		...
		if (p_mpinfo[i]->bgp_nhg_nexthop != p_nhg_childs[i]) {
			...
			p_mpinfo[i]->bgp_nhg_nexthop = p_nhg_childs[i];
	...
	bool *allow_recursion)
{
	...
	bgp_zebra_nexthop_group_configure(info, p, bgp, api, valid_nh_count,
					  nhg_id, allow_recursion, p_mpinfo);
}
