				      ...
				      const struct prefix *p, int debug)
{
	...
	if (bpi_ultimate->sub_type == BGP_ROUTE_REDISTRIBUTE ||
		 ...
		 * with 'nexthop vpn export' command. Use the source_bpi
	...
	if (to_bgp->srv6_enabled &&
	    ((!new_attr->srv6_l3vpn && !new_attr->srv6_vpn) ||
	     (bgp_nexthop->vpn_policy[afi].tovpn_sid == NULL && bgp_nexthop->tovpn_sid == NULL) ||
	     (bgp_nexthop->tovpn_sid_index == 0 &&
