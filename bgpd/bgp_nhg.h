...
PREDECL_RBTREE_UNIQ(bgp_nhg_cache);
...
#define BGP_NHG_STATE_REMOVED	(1 << 1)
	 ...
	 */
	LIST_HEAD(nhg_depend_list, bgp_nhg_cache) depends;

	LIST_ENTRY(bgp_nhg_cache) dependents_thread;

	struct bgp_nhg_cache *parent_nhg;
	unsigned int depends_count;
};

extern int bgp_nhg_cache_compare(const struct bgp_nhg_cache *a,
				 const struct bgp_nhg_cache *b);
DECLARE_RBTREE_UNIQ(bgp_nhg_cache, struct bgp_nhg_cache, entry,
		    bgp_nhg_cache_compare);
