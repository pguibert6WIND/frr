...
PREDECL_RBTREE_UNIQ(bgp_nhg_cache_id);
PREDECL_RBTREE_UNIQ(bgp_nhg_connected_tree);

extern struct bgp_nhg_cache_head nhg_cache_table;
extern struct bgp_nhg_cache_id_head nhg_cache_id_table;
...
struct bgp_nhg_cache {
	...
	struct bgp_nhg_cache_id_item id_entry;
...
#define BGP_NHG_STATE_MODIFIED	(1 << 2)
	uint16_t state;
	 ...
	 * bgp_nhg(3)->depends has 1 and 2 in the tree
	 * bgp_nhg(3)->dependents_thread is empty
	 *
	 * bgp_nhg(1)->depends is empty
	 * bgp_nhg(1)->dependents_thread is 3 in the tree
	 *
	 * bgp_nhg(2)->depends is empty
	 * bgp_nhg(2)->dependents_thread is 3 in the tree
	 */
...
DECLARE_HASH(bgp_nhg_cache, struct bgp_nhg_cache, entry, bgp_nhg_cache_compare,
	     bgp_nhg_cache_hash);
extern int bgp_nhg_cache_id_compare(const struct bgp_nhg_cache *a,
				    const struct bgp_nhg_cache *b);
DECLARE_RBTREE_UNIQ(bgp_nhg_cache_id, struct bgp_nhg_cache, id_entry,
		    bgp_nhg_cache_id_compare);
