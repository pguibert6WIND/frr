...
void nexthop_add_srv6_seg6local(struct nexthop *nexthop, uint32_t action,
				const struct seg6local_context *ctx, struct in6_addr *segs,
				int num_segs)
{
	int i;
	...
	if (num_segs > 0) {
		...
		if (!nexthop->nh_srv6->seg6_segs) {
			nexthop->nh_srv6->seg6_segs =
				XCALLOC(MTYPE_NH_SRV6, sizeof(struct seg6_seg_stack) +
			     ...
			     struct nexthop *rparent)
{
	...
	if (nexthop->nh_srv6) {
		if (nexthop->nh_srv6->seg6local_action == ZEBRA_SEG6_LOCAL_ACTION_END_B6_ENCAP &&
		    nexthop->nh_srv6->seg6_segs &&
		    nexthop->nh_srv6->seg6_segs->num_segs > 1)
			nexthop_add_srv6_seg6local(copy, nexthop->nh_srv6->seg6local_action,
						   &nexthop->nh_srv6->seg6local_ctx,
						   &nexthop->nh_srv6->seg6_segs->seg[0],
						   nexthop->nh_srv6->seg6_segs->num_segs);
		else if (nexthop->nh_srv6->seg6local_action != ZEBRA_SEG6_LOCAL_ACTION_UNSPEC)
			nexthop_add_srv6_seg6local(copy, nexthop->nh_srv6->seg6local_action,
						   &nexthop->nh_srv6->seg6local_ctx, NULL, 0);
		else if (nexthop->nh_srv6->seg6_segs && nexthop->nh_srv6->seg6_segs->num_segs &&
			 !sid_zero(nexthop->nh_srv6->seg6_segs))
