...
struct nexthop *nexthop_from_zapi_nexthop(const struct zapi_nexthop *znh)
{
	...
	if (znh->seg6local_action != ZEBRA_SEG6_LOCAL_ACTION_UNSPEC)
		nexthop_add_srv6_seg6local(n, znh->seg6local_action, &znh->seg6local_ctx, NULL, 0);
			      ...
			      const struct nexthop *nh)
{
	...
	if (nh->nh_srv6) {
		if (nh->nh_srv6->seg6local_action != ZEBRA_SEG6_LOCAL_ACTION_UNSPEC &&
		    nh->nh_srv6->seg6local_action != ZEBRA_SEG6_LOCAL_ACTION_END_B6_ENCAP) {
			SET_FLAG(znh->flags, ZAPI_NEXTHOP_FLAG_SEG6LOCAL);
			znh->seg6local_action = nh->nh_srv6->seg6local_action;
			memcpy(&znh->seg6local_ctx,
			       &nh->nh_srv6->seg6local_ctx,
			       sizeof(struct seg6local_context));
		} else if (nh->nh_srv6->seg6local_action == ZEBRA_SEG6_LOCAL_ACTION_END_B6_ENCAP &&
			   nh->nh_srv6->seg6_segs && nh->nh_srv6->seg6_segs->num_segs > 0) {
			SET_FLAG(znh->flags, ZAPI_NEXTHOP_FLAG_SEG6LOCAL);
			znh->seg6local_action = nh->nh_srv6->seg6local_action;
			...
			for (i = 0; i < nh->nh_srv6->seg6_segs->num_segs; i++)
				memcpy(&znh->seg6_segs[i], &nh->nh_srv6->seg6_segs->seg[i],
				       sizeof(struct in6_addr));
		} else if ((nh->nh_srv6->seg6local_action == ZEBRA_SEG6_LOCAL_ACTION_UNSPEC) &&
			   nh->nh_srv6->seg6_segs && nh->nh_srv6->seg6_segs->num_segs &&
